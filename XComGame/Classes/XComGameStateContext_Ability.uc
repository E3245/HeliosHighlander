...
enum ECinescriptPerContextOverride
{
	eOverride_NoOverride,			//Default value, use the standard logic
	eOverride_NeverCinescriptCamera,
	eOverride_AlwaysCinescriptCamera,
};

var AbilityInputContext     InputContext;
var AbilityResultContext    ResultContext;

// transient vars set on this context object during visualization processing - not to be used outside of ContextBuildVisualization
var transient ECinescriptPerContextOverride CinescriptCameraOverride; //Allows for specific uses of an ability to control whether the cinescript camera is used or not
...
function AbilityPostActivationUpdates(XComGameState NewGameState)
{
	...
	if (SourceUnitState.GetAssociatedPlayerID() > 0)
	{
	...
	out X2Action InOutStartNode, out X2Action InOutEndNode)
{
	...
	//If the vis sequence processing wants to skip the cinescript camera
	if (CinescriptCameraOverride == eOverride_NeverCinescriptCamera)
	{
		return;
	}
	
	//If no override is specified then run through the battery of tests for whether we should do a cinescript camera
	if (CinescriptCameraOverride == eOverride_NoOverride)
	{
		...
		// During breach mode, alert abilities shouldn't affect cameras
		if (`BATTLE.GetDesc().bInBreachPhase && (AbilityTemplate.DataName == 'RedAlert' || AbilityTemplate.DataName == 'YellowAlert'))
		{
			return;
		}

		//Don't add a cinescript camera if the camera is being controlled by the breach action sequencer. If the sequencer should use a cinescript camera, it should do it directly
		if (class'XComTacticalGRI'.static.GetBreachActionSequencer().IsSequencedBreachAction(self))
		{
			return;
		}
	}
	...
	{	
		...
		CinescriptEndAction.StartAction = CinescriptStartAction;
	}
}
	...
	out X2Action InOutStartNode, out X2Action InOutEndNode)
{
	...
	if( (SourceUnitState.IsPlayerControlled() && AbilityTemplate.FrameAbilityCameraType != eCameraFraming_Always)
		|| !ShouldFrameAbility() )
	{
		return;
	}
...
function bool ShouldFrameAbility()
{
	...
	//Dio: Don't frame abilities that are used during breach selection. Example: Enemy unit abilities at start of match
	if (BattleDataState.IsInBreachSelection())
	{
		return false;
	}
	...
	if(class'XComTacticalGRI'.static.GetReactionFireSequencer().IsReactionFire(self) || class'XComTacticalGRI'.static.GetBreachActionSequencer().IsSequencedBreachAction(self))
	{
		return false;
	}
...
function string SummaryString()
{
	...
	OutputString = OutputString @  " ->" @ ((InputContext.PrimaryTarget.ObjectID > 0) ? `XCOMHISTORY.GetGameStateForObjectID(InputContext.PrimaryTarget.ObjectID).SummaryString() : "No Target") @ ")";
...
// todo: remove after nativizing
event XComGameStateContext_Ability NativeAccess_BuildContextFromAbility(XComGameState_Ability AbilityState, int PrimaryTargetID, optional array<int> AdditionalTargetIDs, optional array<vector> TargetLocations, optional X2TargetingMethod TargetingMethod)
{
	return BuildContextFromAbility(AbilityState, PrimaryTargetID, AdditionalTargetIDs, TargetLocations, TargetingMethod);
}
...
static function vector FindNearbyBreachRoomLocation(vector Location, int BreachRoomID)
{
	local array<vector> Offsets;
	local vector TestLocation;
	local vector HeightOffset;
	local vector Offset;
	local float StepSize;
	local XComWorldData WorldData;
	local float TileDistance;
	StepSize = class'XComWorldData'.const.WORLD_StepSize;
	WorldData = class'XComWorldData'.static.GetWorldData();
	HeightOffset.X=0.0f;
	HeightOffset.Y=0.0f;
	HeightOffset.Z=class'XComWorldData'.const.WORLD_HalfFloorHeight;
	Offsets.AddItem(vect( 1, 0, 0 ));
	Offsets.AddItem(vect( -1, 0, 0 ));
	Offsets.AddItem(vect( 0, 1, 0 ));
	Offsets.AddItem(vect( 0, -1, 0 ));
	for(TileDistance = 3.0f; TileDistance >=1.0f; TileDistance -= 1.0f)
	{
		foreach Offsets(Offset)
		{
			TestLocation = Location + (TileDistance * StepSize * Offset);
			if(WorldData.GetRoomIDByLocation(TestLocation + HeightOffset) == BreachRoomID)
			{
				return TestLocation;
			}
		}
...
static function XComGameStateContext_Ability BuildContextFromBreachAbility(XComGameState_Ability AbilityState)
{
	local XComGameStateHistory History;
	local XComGameStateContext OldContext;
	local XComGameStateContext_Ability AbilityContext;
	local XComGameState_Unit SourceUnitState, MainBreachSlotUnit;
	local XComGameState_Item SourceItemState;
	local X2AbilityTemplate AbilityTemplate;
	...
	local XGUnit UnitVisualizer;
	local name AoEProfileName;
	local XComGameState_BattleData BattleData;

	local BreachPointInfo PointInfo;
	local BreachSlotInfo SlotInfo;
	local TTile FormationTile;
	local vector GrenadeTargetLocation;
	local bool bUseGrenadeTargetLocation;
	...
	local array<TTile> MeleePathTiles;
	...
	TargetingMethod = new AbilityTemplate.TargetingMethod;
	TargetingMethod.InitFromState(AbilityState);
	Weapon = (SourceItemState != None) ? XComWeapon(XGWeapon(SourceItemState.GetVisualizer()).m_kEntity) : None;
	WeaponTemplate = (SourceItemState != None) ? X2WeaponTemplate(SourceItemState.GetMyTemplate()) : None;


	if( TargetingMethod.UseGrenadePath() ) //ad one more flag for breach use main slot formation location
	{
		PointInfo = class'XComBreachHelpers'.static.GetUnitBreachPointInfo(SourceUnitState);
		foreach PointInfo.BreachSlots(SlotInfo)
		{
			if (SlotInfo.bIsMainSlot)
			{
				MainBreachSlotUnit = XComGameState_Unit(History.GetGameStateForObjectID(SlotInfo.BreachUnitObjectRef.ObjectID));
				if( MainBreachSlotUnit != None )
				{
					class'XComBreachHelpers'.static.GetUnitBreachFormationLOSTile(MainBreachSlotUnit,FormationTile);
					GrenadeTargetLocation = `XWORLD.GetPositionFromTileCoordinates( FormationTile );
					bUseGrenadeTargetLocation = true;
					break;
				}
			}
		}
	...
	//find a AOEprofile that matches this ability name
	UnitVisualizer = XGUnit(SourceUnitState.GetVisualizer());
	if(UnitVisualizer!= None && UnitVisualizer.m_kBehavior != None)
	{
		AoEProfileName =  UnitVisualizer.m_kBehavior.GetTargetingProfileFromAbility(AbilityState.GetMyTemplateName());
	}
	if( bUseGrenadeTargetLocation )
	{
		AbilityContext.InputContext.TargetLocations.AddItem( GrenadeTargetLocation );
		Path = `PRECOMPUTEDPATH;
		Path.SetWeaponAndTargetLocation(Weapon, Weapon.m_kPawn.m_eTeam, AbilityContext.InputContext.TargetLocations[0], WeaponTemplate.WeaponPrecomputedPathData);
		Path.CalculateTrajectoryToTarget(WeaponTemplate.WeaponPrecomputedPathData);
		AbilityState.GatherAdditionalAbilityTargetsForLocation(AbilityContext.InputContext.TargetLocations[0], AvailableTargets);
	}
	else if(AoEProfileName != '' && Weapon != None && WeaponTemplate != None)
	{
		if(UnitVisualizer.m_kBehavior.FindAoETarget(AoEProfileName) )// && UnitVisualizer.m_kBehavior.BT_SelectAoETarget(AoEProfileName))
		{
			if(!TargetingMethod.UseGrenadePath())
			{
				AbilityContext.InputContext.TargetLocations.AddItem( UnitVisualizer.m_kBehavior.TopAoETarget.Location );
			}
		}
		if(AbilityContext.InputContext.TargetLocations.length == 0)
		{
			//cant find an aoe target location - find a spot in the breach room
			AbilityContext.InputContext.TargetLocations.AddItem(FindNearbyBreachRoomLocation(UnitVisualizer.Location, BattleData.BreachingRoomID));
			if(TargetingMethod.UseGrenadePath())
			{
				Path = `PRECOMPUTEDPATH;
				Path.SetWeaponAndTargetLocation(Weapon, Weapon.m_kPawn.m_eTeam, AbilityContext.InputContext.TargetLocations[0], WeaponTemplate.WeaponPrecomputedPathData);
	...
	else
	{
		...
		if( AvailableTargetList.length > 0 )
		{
			...
			if (TargetingMethod.UseGrenadePath() && Weapon != None && WeaponTemplate != None)
			{
				Path = `PRECOMPUTEDPATH;
				Path.SetWeaponAndTargetLocation(Weapon, Weapon.m_kPawn.m_eTeam, AbilityContext.InputContext.TargetLocations[0], WeaponTemplate.WeaponPrecomputedPathData);
				Path.CalculateTrajectoryToTarget(WeaponTemplate.WeaponPrecomputedPathData);
		...
		else if (AbilityTemplate.bRequireTargetDuringBreach)
		{
			//We don't have a valid ability target but require it, ignore this context request
			return None;
		}
	}
	...
	//Fill out multi shooter information specific to breach abilities
	AbilityState.RebuildMultiShooterOptionsForTarget(AvailableTargets, SourceUnitState);
	for (Index = 0; Index < AvailableTargets.AdditionalShooters.length; Index++)
	{
		AbilityContext.InputContext.MultiShooters.AddItem(AvailableTargets.AdditionalShooters[Index]);
	}
	...
	if (AbilityTemplate.AbilityToHitCalc != none)
	{
		...
		CheckTargetForHitModification(AvailableTargets, AbilityContext, AbilityTemplate, AbilityState);
	}

	// special handling for cone targeting method
	if (X2TargetingMethod_Cone(TargetingMethod) != none)
	{
		X2TargetingMethod_Cone(TargetingMethod).GetReticuleTargets(AbilityContext.InputContext.VisibleTargetedTiles, AbilityContext.InputContext.VisibleNeighborTiles);
	}
	...
	// special handling for pathing targeting methods, check if this ability requires a movement path but didn't provide one
		...
		AbilityTemplate.TargetingMethod.default.ProvidesPath)
	{
		if (class'X2AbilityTarget_MovingMelee'.static.SelectAttackTile(SourceUnitState,
			History.GetGameStateForObjectID(AbilityContext.InputContext.PrimaryTarget.ObjectID),
			AbilityTemplate,
			MeleeDestinations))
		{
			XGUnit(SourceUnitState.GetVisualizer()).m_kReachableTilesCache.BuildPathToTile(MeleeDestinations[0], MeleePathTiles);
		}
	}

	if (MeleePathTiles.Length > 1 && !AbilityTemplate.bRenderTargetingPathOnlyDontSubmitMove)
	{
		// we have a movement path as part of this ability, so add it
		PathData.MovementTiles = MeleePathTiles;
		PathData.MovingUnitRef.ObjectID = AbilityContext.InputContext.SourceObject.ObjectID;
		class'XComTacticalController'.static.CreatePathDataForPathTiles(PathData);
		AbilityContext.InputContext.MovementPaths.AddItem(PathData);

		Destination = MeleePathTiles[MeleePathTiles.Length - 1];
		...
		{
			MeleePathTiles.Remove(0, MeleePathTiles.Length);
			...
			CosmeticUnitVisualizer.m_kReachableTilesCache.BuildPathToTile(Destination, MeleePathTiles);

			if (MeleePathTiles.Length == 0)
			{
				class'X2PathSolver'.static.BuildPath(AttachedUnits[Index], AttachedUnits[Index].TileLocation, Destination, MeleePathTiles);
			}
	...
	// setup movement result contexts, fill them out here.
	while (AbilityContext.ResultContext.PathResults.Length < AbilityContext.InputContext.MovementPaths.Length)
	{
		class'X2TacticalVisibilityHelpers'.static.FillPathTileData(AbilityContext.InputContext.MovementPaths[AbilityContext.ResultContext.PathResults.Length].MovingUnitRef.ObjectID,
			AbilityContext.InputContext.MovementPaths[AbilityContext.ResultContext.PathResults.Length].MovementTiles,
			PathingResult.PathTileData);
		AbilityContext.ResultContext.PathResults.AddItem(PathingResult);
	}
			...
			if (TargetUnitState.Untouchable > 0)                                       //  untouchable is used up from any attack
			{
				if (TargetUnitState.ControllingPlayer.ObjectID != `TACTICALRULES.GetCachedUnitActionPlayerRef().ObjectID)
				{
					...
					`COMBATLOG("*Untouchable preventing a hit against" @ TargetUnitState.GetName(eNameType_RankFull));
					  ...
					  TargetUnitState.IsUnitAffectedByEffectName(class'X2Ability'.default.CounterattackDodgeEffectName) &&
					  class'Engine'.static.GetEngine().SyncRand(100, "CounterattackRoll") < class'X2Ability_Muton'.default.COUNTERATTACK_DODGE_AMOUNT)
			{
...
static function XComGameStateContext_Ability ActivateAbility_Internal(ActionSelection SelectAction, optional int VisualizeAtHistoryIndex = -1, optional SequencePlayTiming InAssociatedPlayTiming)
{
	...
	if( SelectAction.AvailableTargetsIndex > -1 && SelectAction.AvailableTargetsIndex < SelectAction.PerformAction.AvailableTargets.Length)
	{
	...
	else
	{
		...
		//  An ability without targets - e.g. Grapple, Flamethrower
	...
	// Start HELIOS Issue #60 (WotC CHL #890)
	...
	// End HELIOS Issue #60 (WotC CHL #890)
	...
	if( SelectAction.PathTiles.Length > 1 && !AbilityTemplate.bRenderTargetingPathOnlyDontSubmitMove)
	{
		// we have a movement path as part of this ability, so add it
	...
	PlayerState = XComGameState_Player(History.GetGameStateForObjectID(SourceUnitState.ControllingPlayer.ObjectID));
	if (PlayerState.IsAIPlayer())
	{
		AbilityTemplate.PlayAIAbilityConfirmSoundDeferred();
	}
	else
	{
		AbilityTemplate.PlayAbilityConfirmSound();
	}
...
function name FindShooterEffectApplyResult(const X2Effect Effect)
{
	...
	for (i = 0; i < ResultContext.ShooterEffectResults.Effects.Length; ++i)
	{
		...
		if (Effect.bFindEffectResultsByInexactMatchUsingClass && Effect.Class == ResultContext.ShooterEffectResults.Effects[i].Class)
		{
			return ResultContext.ShooterEffectResults.ApplyResults[i];
		}
	}
...
function name FindTargetEffectApplyResult(const X2Effect Effect)
{
	...
	for (i = 0; i < ResultContext.TargetEffectResults.Effects.Length; ++i)
	{
		...
		if (Effect.bFindEffectResultsByInexactMatchUsingClass && Effect.Class == ResultContext.TargetEffectResults.Effects[i].Class)
		{
			return ResultContext.TargetEffectResults.ApplyResults[i];
		}
	}
...
function name FindMultiTargetEffectApplyResult(const X2Effect Effect, const int TargetIndex)
{
	...
	for (i = 0; i < ResultContext.MultiTargetEffectResults[TargetIndex].Effects.Length; ++i)
	{
		...
		if (Effect.bFindEffectResultsByInexactMatchUsingClass && Effect.Class == ResultContext.MultiTargetEffectResults[TargetIndex].Effects[i].Class)
		{
			return ResultContext.MultiTargetEffectResults[TargetIndex].ApplyResults[i];
		}
	}
