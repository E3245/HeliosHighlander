...
var bool			m_bShowingPenalties;
var bool			m_bForceFlushPenalties;
...
var localized string m_strStatus;
var localized string m_strDebuff;
var localized string m_BreachSurpriseFriendly;
var localized string m_BreachRepositionFriendly;
var localized string m_BreachBehaviorFriendly;
var localized string m_BreachFireFriendly;
...
// TEMP
native function bool InGameThread();
...
simulated function OnInit()
{
	...
	{
		StartingState = History.GetGameStateForObjectID(StoredObjectID);
	}
...
simulated function RespondToNewGameState(XComGameState NewState, bool bForceUpdate = false)
{
	...
	local bool bManualTargetBreachContext;
	...
	{
		bManualTargetBreachContext = XComGameStateContext_BreachSequence(NewState.GetContext()) != None && XComGameStateContext_BreachSequence(NewState.GetContext()).AbilityName != '';
		if( NewState != None && !bManualTargetBreachContext)
		{
...
simulated function UpdateFromDestructibleState(XComGameState_Destructible NewDestructibleState, bool bInitialUpdate = false, bool bForceUpdate = false)
{
	...
	if( DestructibleActor != none )
	{
		...
		SetHitPoints(NewDestructibleState.Health, TotalHealth, false);

		// grenade info for UI
		if ( NewDestructibleState.bIsFused) // is this a grenade?
		{
			SetNames(NewDestructibleState.SpawnedDestructibleLocName, "");
			AS_SetTurnCount(string(NewDestructibleState.GetNumTurnRemainingToExplode()));
		}
...
simulated function UpdateFromUnitState(XComGameState_Unit NewUnitState, bool bInitialUpdate = false, bool bForceUpdate = false)
{
	...
	RealizeHitPoints(NewUnitState, bForceUpdate || bInitialUpdate );
	...
	if( bInitialUpdate )
	{
		...
		SoldierClassTemplate = NewUnitState.GetSoldierClassTemplate();
		SetClassIcon(SoldierClassTemplate != none ? SoldierClassTemplate.IconImage : "img:///UILibrary_Common.TargetIcons.target_alien");
	}
...
simulated function Update(XGUnit kNewActiveUnit)
{
	...
	local XComGameState_Unit UnitState, VisualizedUnitState, ActiveUnitState;
	...
	// TEMPORARY CLOOGE FOR FIRST PLAYABLE.*********
	local array<UISummary_UnitEffect> Penalties;
	local int i;
	local bool bShowPenaltyPersistentDueToBreach;
	local bool bShowPenalties, bHidePenalties;
	...
	else
	{
		...
		else
		{
			...
			if( m_kReticle != none || m_bIsSelected )
				...
				flagScale = (unitScreenPos.Y / 22.5) + 58.0;
	...
	//In Breach, show ALL unit flag penalties always
	BattleData = XComGameState_BattleData(History.GetSingleGameStateObjectForClass(class'XComGameState_BattleData'));
	bShowPenaltyPersistentDueToBreach = BattleData != None && BattleData.bInBreachPhase;
	bShowPenalties = (kNewActiveUnit == VisualizedActor) || bShowPenaltyPersistentDueToBreach;
	bHidePenalties = !bShowPenalties && (kNewActiveUnit != none);
	if (bShowPenalties && (!m_bShowingPenalties || m_bForceFlushPenalties))
	{

		VisualizedUnitState = XComGameState_Unit(History.GetGameStateForObjectID(VisualizedUnit.ObjectID));
		Penalties = VisualizedUnitState.GetUISummary_UnitEffectsByCategory(ePerkBuff_Penalty);

		m_bForceFlushPenalties = false;

		if (Penalties.length > 0)
		{
			m_bShowingPenalties = true;

			for (i = 0; i < Penalties.length; i++)
			{
				AS_SetMessage(m_strDebuff, Penalties[i].Name, bShowPenaltyPersistentDueToBreach);
			}
	...
	else if (bHidePenalties && m_bShowingPenalties)
	{
		m_bShowingPenalties = false;

		SetSelected(false);
		HideExtension();

		AS_SetMessage(m_strDebuff, "", bShowPenaltyPersistentDueToBreach);
	}
}
...
simulated function SetFlagPosition(int flagX, int flagY, int scale)
{
	...
	{
		yOffset = `BATTLE.GetDesc().IsInBreachSelection() ? 0 : m_iYOffset;

		m_scale = scale;
		m_positionV2.X = flagX;
		m_positionV2.Y = flagY + yOffset;
...
simulated function SetClassIcon(string classIcon)
{
	local ASValue myValue;
	local Array<ASValue> myArray;
...
simulated function RealizeHitPoints(optional XComGameState_Unit NewUnitState = none, optional bool bForceUpdate =false )
{
	...
	SetHitPoints(NewUnitState.GetCurrentStat(eStat_HP), NewUnitState.GetMaxStat(eStat_HP), NewUnitState.IsUnconscious(), bForceUpdate );
...
static function GetHitPoints(int _currentHP, int _maxHP, out int currentHP, out int maxHP)
{
...
simulated function SetHitPoints(int _currentHP, int _maxHP, bool bUnconscious, optional bool bForceUpdate = false)
{
	local ASValue myValue;
	local Array<ASValue> myArray;
	local int currentHP, maxHP;

	GetHitPoints(_currentHP, _maxHP, currentHP, maxHP);

	if( _currentHP < 1 || bUnconscious)
	{
		m_bIsDead = true;
		Remove();
	}
	else
	{
		...
		else if ( !m_bIsFriendly.GetValue() && bOnlyShowMaxHealth)
		{
			m_currentHitPoints.SetValue(maxHP);
		...
		if( bForceUpdate || m_currentHitPoints.HasChanged() || m_maxHitPoints.HasChanged() )
		{
...
simulated function SetShieldPoints(int _currentShields, int _maxShields)
{
	...
	else if ( !m_bIsFriendly.GetValue() && bOnlyShowMaxHealth)
	{
		...
		m_shieldPoints.SetValue(maxShields);
...
static function int GetArmorPoints(optional int _iArmor = 0 )
{
...
simulated function SetArmorPoints(optional int _iArmor = 0)
{
	...
	{
		currentArmor = GetArmorPoints(_iArmor);
...
simulated function SetHitPointsPreview(optional int _iPossibleDamage = 0, optional int _iPossibleMinDamage = 0)
{
	local ASValue myValue;
	local Array<ASValue> myArray;
...
simulated function SetCoverPointsPreview(int CoverPipsToDisplay)
{
	local ASValue myValue;
	local Array<ASValue> myArray;
	...
	//----------------------
	myValue.Type = AS_String;
	myValue.s = GetCoverPipStringForCoverType(CT_None, CoverPipsToDisplay);
	myArray.AddItem(myValue);
...
static function string GetCoverPipStringForCoverType(ECoverType CoverType, optional int NumPipsToDisplay = -1)
{
	...
	if (CoverType != CT_None)
	{
		...
		{
			//RetString = "_highCover"; //dakota.lemaster(8.22.19) Hack Flash into thinking this is low cover, so the correct number of coverarmor pips show
			RetString = "_lowCover";
		}
		else
		{
			//RetString = "_lowCover"; //dakota.lemaster(8.22.19) Hack Flash into thinking this is no cover, so the correct number of coverarmor pips show
		}
	}

	if (NumPipsToDisplay > 0)
	{
		RetString = "_lowCover"; // tell flash we're in low cover, aka one pip shown
	}
...
simulated function RealizeCover(optional XComGameState_Unit UnitState = none, optional int HistoryIndex = INDEX_NONE)
{
	...
	//dakota.lemaster: Only show cover pips during shot previews
	//if( UnitState != none )
	//{
	//	CoverType = UnitState.GetCoverTypeFromLocation();
	//	myValue.s = GetCoverPipStringForCoverType(CoverType);
	//}
...
simulated function SetHovered(bool isHovered)
{
	local ASValue myValue;
	local Array<ASValue> myArray;
	...
	myValue.Type = AS_Boolean;
	myValue.b = m_bisHovered;
	myArray.AddItem(myValue);

	Invoke("SetHovered", myArray);
...
{
	return; //Stop fading anything out! GUH! -bsteiner 7.30.2019
...
simulated function RealizeOverwatch(optional XComGameState_Unit NewUnitState = none)
{
	...
	{
		AS_SetOverwatchIcon( 
			( NewUnitState.ReserveActionPoints.Find( 'Overwatch' ) > -1
			|| NewUnitState.ReserveActionPoints.Find( class'X2Ability_SharpshooterAbilitySet'.default.KillZoneReserveType ) > -1 )
		);
	}
}
...
simulated function RealizeDebuffs(optional XComGameState_Unit NewUnitState = none)
{
	...
	if( NewUnitState != none )
	{
		m_bForceFlushPenalties = true;
	}
}

simulated function AS_SetMessage(string str0, string str1, bool bPersistent = false)
{
...
simulated function RealizeStatus(optional XComGameState_Unit NewUnitState = none)
{
	...
	local int ActiveUnitObjectID;
	local EUIState BGColorState;
	local bool bShowBreachReaction;
	local EBreachEnemyIntent EnemyIntent;
	bShowBreachReaction = false;
	...
	AS_SetStatusIcons(NewUnitState.GetUISummary_UnitStatusIcons());

	//Enemies during the breach sequence are either surprised or prepared
	if (default.bDisplaySurprisedPreparedDuringBreach)
	{
		if (NewUnitState.GetTeam() != eTeam_XCom && `BATTLE.GetDesc().bInBreachPhase && !`BATTLE.GetDesc().IsInBreachSelection())
		{
			//HACK: piggy-backing on the downed-ui until a separate ui is created

			if (class'XComBreachHelpers'.static.CanEnemyIntentBePerformed(NewUnitState))
			{
				EnemyIntent = class'XComBreachHelpers'.static.GetEnemyIntent(NewUnitState.ObjectID);

				foreach History.IterateContextsByClassType(class'XComGameStateContext_BreachSequence', BreachTargetingContext, , true)
				{
					ActiveUnitObjectID = BreachTargetingContext.UnitRef.ObjectID;
					break;
				}

				BGColorState = class'UIUtilities_Colors'.static.GetEnemyBreachIntentColorState(ActiveUnitObjectID, NewUnitState.ObjectID);
				
				switch (EnemyIntent)
				{
				case eBreachEnemyIntent_Surprise:
					AS_SetBreachReaction(default.m_BreachSurpriseFriendly, BGColorState);
					break;
				case eBreachEnemyIntent_Reposition:
					AS_SetBreachReaction(default.m_BreachRepositionFriendly, BGColorState);
					break;
				case eBreachEnemyIntent_Behavior:
					AS_SetBreachReaction(default.m_BreachBehaviorFriendly, BGColorState);
					break;
				case eBreachEnemyIntent_Fire:					
					AS_SetBreachReaction(default.m_BreachFireFriendly, BGColorState);
					break;
				default:
					AS_SetBreachReaction(default.m_BreachSurpriseFriendly, BGColorState);
					break;
				...
				bShowBreachReaction = true;
			}
			else
			{
				//@brit.steiner, why won't the flag hide during breach?? from: @dakota.lemaster
				//AS_SetBreachReaction("", eUnitFlagMessageStyle_Default); //dakota.lemaster: even tried this as overkill to no success
			}
		}
	}

	if (!bShowBreachReaction)
	{
		Invoke("RemoveBreachReaction");
	}
}

simulated function AS_SetBreachReaction(string locEffectName, EUIState UIStateColor)
{
	local ASValue myValue;
	local Array<ASValue> myArray;

	myValue.Type = AS_String;
	myValue.s = m_strStatus;
	myArray.AddItem(myValue);

	myValue.Type = AS_String;
	myValue.s = locEffectName;
	myArray.AddItem(myValue);

	myValue.Type = AS_String;
	myValue.s = class'UIUtilities_Colors'.static.GetHexColorFromState(UIStateColor);
	myArray.AddItem(myValue);

	myValue.Type = AS_Boolean;
	myValue.b = UIStateColor == eUIState_Response_Aggressive;
	myArray.AddItem(myValue);

	Invoke("SetBreachReaction", myArray);
}

simulated function AS_SetStatusIcons(array<string> StatusIcons)
{
	//local ASValue myValue;
	//local Array<ASValue> myArray;
	//local string iconString;
	local GFxObject gfxIcons;
	local int i;
	
	gfxIcons = Movie.CreateArray();
	for (i = 0; i < StatusIcons.length; i++)
	{
		gfxIcons.SetElementString(i, StatusIcons[i]);
	}
	
	SendStatusIcons(gfxIcons);
}

function SendStatusIcons(GFxObject reinforcementData)
{
	RootMC.ActionScriptVoid("SetStatus");
}
...
// Show either turn info or aggression status string 
simulated function RealizeTurnInfo(XComGameState_Unit NewUnitState)
{
	local int TurnNumber;

	TurnNumber = `PRES.m_kTacticalTimeline.GetUnitIndexInNonPreviewMarkerEntries( NewUnitState.ObjectID );
	if( !`BATTLE.GetDesc().bInBreachPhase && TurnNumber != INDEX_NONE )
	{
		AS_SetTurnCount( string( TurnNumber + 1 ) );
	}
	else
	{
		//AS_SetTurnCount("");
...
simulated function AS_SetTurnCount(string displayStr, bool bIntroAnimate = false, bool bChangeAnimate = false )
{
	local ASValue myValue;
	local Array<ASValue> myArray;

	myValue.Type = AS_String;
	myValue.s = displayStr;
	myArray.AddItem(myValue);

	myValue.Type = AS_Boolean;
	myValue.b = bIntroAnimate;
	myArray.AddItem(myValue);

	myValue.Type = AS_Boolean;
	myValue.b = bChangeAnimate;
	myArray.AddItem(myValue);

	Invoke("SetTurnCount", myArray);
}
...
defaultproperties
{
	...
	m_iYOffset = -75; // gfx y offset when not in breach mode
