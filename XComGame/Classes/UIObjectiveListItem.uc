...
var localized string m_TagStr_encounterNum;
var localized string m_strDarkEventsListTitle;
var localized string m_strPriority;
var localized string m_strInProgress;

var delegate<OnItemChangedDelegate> OnItemChanged;
delegate OnItemChangedDelegate(UIObjectiveListItem Item);
...
simulated function SetTitle(ObjectiveDisplayInfo NewData, optional bool bForce = false)
{
	...
	// HELIOS BEGIN
	// Test for whomever is the correct StrategyHUD else we get log spam about missing BattleState
	if (!`SCREENSTACK.IsInStack(`PRESBASE.UIHUD_Strategy)) // we're in strategy and dont want tactical objectives
	// HELIOS END
	{
		...
		if (BattleData.MapData.RoomIDs.length > 1)
		{
			...
			LocTag.IntValue0 = BattleData.GetCurrentEncounterIndex() + 1;
			LocTag.IntValue1 = BattleData.GetTotalEncounterCount();
			FormattedString = `DIO_UI.static.FormatBody(`XEXPAND.ExpandString(m_TagStr_encounterNum));
		}
		else
		{
			FormattedString = "";
	...
	if( Data != NewData || bForce )
	{
		...
		if( Data.bIsDarkEvent )
			...
			MC.FunctionString("setTitle", m_strObjectivesListTitle $ FormattedString);
	}
}
...
simulated function SetTimerRow(ObjectiveDisplayInfo NewData, optional bool bForce = false)
{
	if(Data != NewData || bForce)
	{
		...
		mc.QueueNumber(NewData.UiState);
...
simulated function SetCheckboxRow(ObjectiveDisplayInfo NewData, optional bool bForce = false)
{
	...
	if(Data != NewData || bForce)
	{
		...
		mc.QueueString(GetDisplayLabelString(NewData));
		mc.QueueString(NewData.MissionType);
		mc.EndOp(); 
	}
}
