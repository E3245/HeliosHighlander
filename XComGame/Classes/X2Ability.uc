...
var config const bool bAllowMatineeVisualizers;					 // DIO - allows an INI file toggle for whether to allow the new matinee visualizers or not
...
var protected X2AbilityCost_ActionPoints	SimpleSingleAPCost, SimpleSingleAPConsumeAllCost;
var protected X2Effect_ApplyWeaponDamage    WeaponUpgradeMissDamage;
var protected X2AbilityCooldown				SimpleCooldownUntilTurnEnd;
//---------------------------------------------------------------------------------------
...
cpptext
{
	UBOOL HasCinematicShot(FName AbilityName); //Returns TRUE if the given AbilityName has an entry in the cinematic shot data. Used to decide whether to play a matinee visualizer
}
...
static function UpdateTargetLocationsFromContext(XComGameStateContext_Ability AbilityContext)
{
	...
	if(AbilityTemplate.AbilityTargetEffects.Length > 0 && AbilityContext.InputContext.PrimaryTarget.ObjectID != 0)
	{
		...
		if(PrimaryTargetVisualizer != none)
		{
			...
			if( AbilityContext.ResultContext.HitResult == eHit_Barrage || AbilityContext.ResultContext.HitResult == eHit_Shield )
			{
				AbilityContext.ResultContext.ProjectileHitLocations.AddItem(TargetShootAtLocation);
	...
	if(AbilityContext.ResultContext.ProjectileHitLocations.Length > 0)
	{
		...
		if((SourceStateObject != none && AbilityStateObject != none))
		{
			...
			if(WeaponTemplate != none)
			{
				...
				EnvironmentDmg = WeaponTemplate.iEnvironmentDamage;
			}
			else
			{
			...
			for(Index = 1; Index < AbilityContext.InputContext.ProjectileEvents.Length; ++Index)
			{
				if(AbilityContext.InputContext.ProjectileEvents[Index].bEntry)
				{
					...
					DamageEvent.DamageAmount = EnvironmentDmg;
					DamageEvent.bAffectFragileOnly = (AbilityTemplate.bFragileDamageOnly || EnvironmentDmg <= 1) ? true : false;
					...
					if (InteractActor != none && InteractActor.IsDoor() && InteractActor.GetInteractionCount() % 2 == 0 && WeaponTemplate.Name != 'Flamethrower' )
					{
						...
						//make sure neither side of the door is in an unbreached room
						if( !`XWORLD.IsUnbreachedDoor( InteractActor ) )
						{
...
static function TypicalAbility_FillOutGameState(XComGameState NewGameState)
{
	...
	local int LoopObjectID, LoopAbilityObjectID;
	local XComGameState_Ability LoopAbilityState;
	local XComGameState_Unit  LoopUnit_OriginalState;
	local XComGameState_Unit  LoopUnit_NewState;
	local XComGameState_Unit  SourceUnit_NewState;

	local UnitValue MetersMovedValue;
	local float MetersMoved;
	...
	SourceUnit_NewState = XComGameState_Unit(SourceObject_NewState);

	//Track tiles moved by the shooter
	foreach AbilityContext.InputContext.MovementPaths(PathInputData)
	{
		if (PathInputData.MovingUnitRef.ObjectID == SourceUnit_NewState.ObjectID)
		{
			MetersMoved = `TILESTOMETERS((PathInputData.MovementTiles.Length - 1));
			if (MetersMoved > 0)
			{
				SourceUnit_NewState.GetUnitValue('MetersMovedThisTurn', MetersMovedValue);
				SourceUnit_NewState.SetUnitFloatValue('MetersMovedThisTurn', MetersMovedValue.fValue + MetersMoved, eCleanup_BeginTurn);
			}
			break;
		}
	}
	...
	//Apply costs to each shooter, with their corresponding abilities
	if (AbilityContext.InputContext.MultiShooters.Length > 0)
	{
		for (ShooterIndex = 0; ShooterIndex < AbilityContext.InputContext.MultiShooters.Length; ++ShooterIndex)
		{
			LoopObjectID = AbilityContext.InputContext.MultiShooters[ShooterIndex].ObjectID;
			LoopUnit_OriginalState = XComGameState_Unit(History.GetGameStateForObjectID(LoopObjectID));
			if (LoopUnit_OriginalState == None)
			{
				continue;
			}
			LoopUnit_NewState = XComGameState_Unit(NewGameState.ModifyStateObject(LoopUnit_OriginalState.Class, LoopObjectID));
			LoopAbilityObjectID = LoopUnit_OriginalState.FindAbility(AbilityContext.InputContext.AbilityTemplateName).ObjectID;
			if (LoopAbilityObjectID == 0)
			{
				continue;
			}
			LoopAbilityState = XComGameState_Ability(History.GetGameStateForObjectID(LoopAbilityObjectID));
			if (LoopAbilityState == None)
			{
				continue;
			}
			LoopAbilityState = XComGameState_Ability(NewGameState.ModifyStateObject(LoopAbilityState.Class, LoopAbilityState.ObjectID));

			AbilityTemplate.ApplyCost(AbilityContext, LoopAbilityState, LoopUnit_NewState, None, NewGameState);
		}
	}
	//big hack for deflect and subservience
	if (AbilityContext.InputContext.PrimaryTarget.ObjectID != 0 && AbilityContext.ResultContext.HitResult == eHit_Deflect )
	{
		AffectedTargetObject_OriginalState = History.GetGameStateForObjectID(AbilityContext.InputContext.PrimaryTarget.ObjectID, eReturnType_Reference);
		MarkSubservienceEffectOnDeflect( AffectedTargetObject_OriginalState );
		`XEVENTMGR.TriggerEvent('HitResultIs_Deflect', AffectedTargetObject_OriginalState, AffectedTargetObject_OriginalState, NewGameState);
	}
	for( TargetIndex = 0; TargetIndex < AbilityContext.InputContext.MultiTargets.Length; ++TargetIndex )
	{
		if ( AbilityContext.InputContext.MultiTargets[TargetIndex].ObjectID != 0 && AbilityContext.ResultContext.MultiTargetHitResults[TargetIndex] == eHit_Deflect )
		{
			AffectedTargetObject_OriginalState = History.GetGameStateForObjectID(AbilityContext.InputContext.MultiTargets[TargetIndex].ObjectID, eReturnType_Reference);
			MarkSubservienceEffectOnDeflect( AffectedTargetObject_OriginalState );
			`XEVENTMGR.TriggerEvent('HitResultIs_Deflect', AffectedTargetObject_OriginalState, AffectedTargetObject_OriginalState, NewGameState);
...
static function MarkSubservienceEffectOnDeflect( XComGameState_BaseObject BaseObjectState )
{
	local XComGameState_Unit  UnitState, EffectOwnerState;
	local XComGameState_Effect EffectState;
	local int Index;

	UnitState = XComGameState_Unit(BaseObjectState);
	if (UnitState != None)
	{
		Index = UnitState.AffectedByEffectNames.Find( class'X2Effect_Subservience'.default.EffectName );
		if( Index != INDEX_NONE )
		{
			EffectState = XComGameState_Effect( `XCOMHISTORY.GetGameStateForObjectID( UnitState.AffectedByEffects[Index].ObjectID ) );
			EffectOwnerState = ( EffectState != None ) ?  XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(EffectState.ApplyEffectParameters.SourceStateObjectRef.ObjectID)) : None;
			if( EffectOwnerState != None )
			{
				EffectOwnerState.SetUnitFloatValue('SubServienceMarkedForRemoval', 1, eCleanup_BeginTurn);
			}
		}
	}
}
...
static native function bool DidAnyTargetUnitDie(XComGameStateContext_Ability InContext);
/*
...
static native function bool AbilityCanUseMatineeVisualizer(Name AbilityTemplateName);

static native function TypicalAbility_BuildVisualization(XComGameState VisualizeGameState);

static native function TypicalAbility_BuildVisualization_Matinee(XComGameState VisualizeGameState);

//DIO - RAM - Slated to be made native. This is currently called on the CDO by TypicalAbility_BuildVisualization
event TypicalAbility_BuildVisualization_Traditional(XComGameState VisualizeGameState)
{	
	//general
	local XComGameStateHistory	History;
	...
	if(class'X2Ability_BreachAbilities'.default.SKIP_BREACH_VISUALIZATION 
		&& AbilityTemplate.bBreachAbility)
	{
		return;
	}
		...
		AbilityContext.PrimaryTarget.ObjectID > 0)				//There is a primary target
	{
		...
		if(!AbilityTemplate.bSkipFireAction 
			...
			&& !XGUnit(TargetVisualizer).GetPawn().bPlayedDeath)
		{
	...
	{
		SoundAndFlyOver = X2Action_PlaySoundAndFlyOver(class'X2Action_PlaySoundAndFlyover'.static.AddToVisualizationTree(SourceData, Context));
	...
	//Broken out into its own method for use by other visualizer code
	class'X2Ability'.static.AddEnvironmentalDamageVisualizerActions(VisualizeGameState, AbilityContext, AbilityTemplate, BuildData, InitData, MultiTargetEffects, ExitCoverAction, WindowBreakTouchIndex);
	...
	foreach VisualizeGameState.IterateByClassType(class'XComGameState_InteractiveObject', InteractiveObject)
	{
		...
		if ( Context.InputContext.MovementPaths.Length > 0 || (InteractiveObject.IsDoor() && InteractiveObject.HasDestroyAnim()) ) //Is this a closed door?
		{
			...
			//If this actor is associated with a window break event, associate it with the appropriate exit cover action.
			if (WindowBreakTouchIndex > -1 &&
				XComInteractiveLevelActor(Context.InputContext.ProjectileEvents[WindowBreakTouchIndex].TraceInfo.HitComponent.Owner) == BuildData.VisualizeActor)
			{
				class'X2Action_BreakInteractActor'.static.AddToVisualizationTree(BuildData, Context, false, ExitCoverAction);
			}
			else
			{
...
static native function AddEnvironmentalDamageVisualizerActions(XComGameState VisualizeGameState, AbilityInputContext AbilityContext, 
	X2AbilityTemplate AbilityTemplate, out VisualizationActionMetadata BuildData, VisualizationActionMetadata InitData, const out array<X2Effect> MultiTargetEffects, X2Action_ExitCover ExitCoverAction, int WindowBreakTouchIndex = -1);
...
simulated function SequentialShot_MergeVisualization(X2Action BuildTree, out X2Action VisualizationTree)
{
	...
	for (i = arrActions.Length - 1; i >= 0; --i)
	{
		...
		// Comment out for Issue WOTC CHL #20
	...
	// Single Line for Issue WOTC CHL #273 Line moved to end so that there will only be one EnterCover is the VisualizationTree so the GetNodesOfType() can't get the wrong one
	...
	// Single Line for Issue WOTC CHL #273 Line moved from earlier
	VisMgr.ConnectAction(MarkerStart, VisualizationTree, true, JoinMarker);
}
...
DefaultProperties
{
	...
	Begin Object Class=X2AbilityCost_ActionPoints Name=DefaultSingleAPCost
		iNumPoints=1
	End Object
	SimpleSingleAPCost = DefaultSingleAPCost;

	Begin Object Class=X2AbilityCost_ActionPoints Name=DefaultSingleAPConsumeAllCost
		iNumPoints = 1
		bConsumeAllPoints=true
	End Object
	SimpleSingleAPConsumeAllCost = DefaultSingleAPConsumeAllCost;
	...
	Begin Object Class=X2AbilityCooldown Name=DefaultEndOfTurnCooldown
		iNumTurns=1
	End Object
	SimpleCooldownUntilTurnEnd = DefaultEndOfTurnCooldown;
