...
var protectedwrite bool           	bInLatentTurn;          //This flag is true during the 'TurnTowardsPosition' latent function
...
var	bool							bMoveFireIdle;			//Instructs the state machine that we are idle during a move fire action
var	bool							bInBreachIdle;			//Fix for HLX 1861 - lets the idle state machine know that the unit should use its breach idle. 
														//Set from BreachSetUnit_BuildVisualization, cleared in X2Action_MoveBegin when the unit moves into the encounter area
//****************************************
...
var protected float						StoredTurnThreshold;
//****************************************
...
simulated function Resume(optional Actor UnlockResume=none, bool bForceDuringAction = false)
{
	...
	if (kDormantLock != none && kDormantLock != UnlockResume)
	{
		...
		`log("Locked by Actor" @ kDormantLock @ "but Resume request was made by" @ UnlockResume @ ", cannot Resume", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
...
simulated function GoDormant(optional Actor LockResume=none, optional bool Force=false, optional bool bDisableWaitingForEval=false)
{
	...
	if (kDormantLock != none && kDormantLock != LockResume)
	{
		...
		`log("Dormant state is locked by" @ kDormantLock @ "but new GoDormant request was made by" @ LockResume @ ", cannot GoDormant", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
	...
	if( Statename != 'EvaluateStance' )
	{
		...
		{			
			`log("*********************** IdleStateMachine GoDormant() ************************************************", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
			`log("Dormant now locked by" @ LockResume,  `CHEATMGR.MatchesXComAnimUnitName(Unit.Name) && LockResume != none, 'XCom_Anim', `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
			`log(GetScriptTrace(), `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
...
function PerformPeek()
{
	...
	BattleData = XComGameState_BattleData(History.GetGameStateForObjectID(BattleData.ObjectID, , UseHistoryIndex));
	...
	if( !bIsCivilian &&
		...
		(BattleData == None || BattleData.IsInBreachSelection() == false) && // dont' perform peeks during breach planning phase
		!`XCOMVISUALIZATIONMGR.IsActorBeingVisualized(UnitState.GetVisualizer())) 
	{
...
function PerformFlinch()
{
	...
	local XComGameState_BattleData BattleData;
	local int UseHistoryIndex;
	local bool bInBreachSequence, bSkipFlinch;
	...
	// No units should flinch during the breach sequence
	bInBreachSequence = (BattleData != None && BattleData.IsInBreachSubPhaseSequenceCamera());
	bSkipFlinch = `DIO_DISABLE_FLINCH_DURING_BREACH && bInBreachSequence;

	if(!bSkipFlinch && IsStateInterruptible() && UnitPawnNative.GetAnimTreeController().GetAllowNewAnimations() )
	{
		GotoState('Flinch');
	}
}
...
function PerformCrouch()
{
	...
	local XComGameState_BattleData BattleData;
	local int UseHistoryIndex;
	local bool bInBreachSequence, bSkipCrouch;
	...
	// No units should flinch during the breach sequence
	bInBreachSequence = (BattleData != None && BattleData.IsInBreachSubPhaseSequenceCamera());
	bSkipCrouch = `DIO_DISABLE_CROUCH_DURING_BREACH && bInBreachSequence;

	if( IsStateInterruptible() && !bTargeting && !bSkipCrouch)
	{
		GotoState('Crouch');
	}
}

function PerformScootOver()
{
	local XComGameStateHistory History;
	local XComGameState_BattleData BattleData;
	local int UseHistoryIndex;
	local bool bInBreachSequence, bSkipScoot;
	...
	// No units should scoot during the breach sequence
	bInBreachSequence = (BattleData != None && BattleData.IsInBreachSubPhaseSequenceCamera());
	bSkipScoot = bInBreachSequence;

	if (!bSkipScoot && IsStateInterruptible() && UnitPawnNative.GetAnimTreeController().GetAllowNewAnimations())
	{
		GotoState('ScootOver');
	}
}
...
{
	local bool bEnemiesPresent;
	local EAlertLevel AlertLevel;
	local float PodRandom;
	local AnimNodeSequence PlayingSequence;
	local int UseHistoryIndex;
	local name BreachIdleAnimName;
	local bool bPlayingPodIdle, bPlayingPodFidget;
	local XComGameStateHistory History;
	local XComGameState_BattleData BattleData;
	local bool bBreachSelectionIdle;
	...
	bEnemiesPresent = UnitState.GetNumVisibleEnemyUnits(true, false,, UseHistoryIndex) > 0;
	...
	if(bIsTurret)
	{
		...
		GetTurretIdleAnim(AnimParams.AnimName, CurrentTurretState, bEnemiesPresent);
	}
	else
	{
		...
		if( AlertLevel == eAL_Green )
		{
			...
			{
				bPlayingPodFidget = UnitPawnNative.GetAnimTreeController().IsPlayingCurrentAnimation('POD_Fidget');
				bPlayingPodIdle = UnitPawnNative.GetAnimTreeController().IsPlayingCurrentAnimation('POD_Idle');
				if((!bPlayingPodFidget && !bPlayingPodIdle) || ResetPodAnimation)
				{
					// Since we want to randomize how the green idle works set a timer to start this animation again
					PlayingSequence = UnitPawnNative.GetAnimTreeController().PlayFullBodyDynamicAnim(AnimParams);
					SetTimer(PlayingSequence.AnimSeq.SequenceLength / fIdleAnimRate, false, nameof(ResetPodIdleAnimation), self);
		...
		if(!UnitPawnNative.GetAnimTreeController().CanPlayAnimation(AnimParams.AnimName))
		{
			...
			if (bEnemiesPresent && !bIsCivilian)
			{
				AlertLevel = eAL_Red;
			}

			//[5.22.19] remain in yellow alert if we're outside the breach and facing our entry point marker @dakota
			if (ShouldFaceEntryPointMarker())
			{
				AlertLevel = eAL_Yellow;
			}
		...
		//If we're in breach mode, play our breach-action specific idle
		if ((bInBreachIdle || BattleData != None && BattleData.IsInBreachSelection()) && ShouldFaceEntryPointMarker())
		{
			if (!UnitState.GetMyTemplate().bIsCosmetic && UnitState.GetTeam() == eTeam_XCom)
			{
				bBreachSelectionIdle = true;
				BreachIdleAnimName = class'XComBreachHelpers'.static.GetUnitBreachIdleAnimName(UnitState.ObjectID);
				if (BreachIdleAnimName != '')
				{
					AnimParams.AnimName = BreachIdleAnimName;
				}
			}
		}
	}

	if( UnitPawnNative != None && AnimParams.AnimName != 'CurrentPose' )
	{
		...
		if (bBreachSelectionIdle)
		{
			AnimParams.BlendTime = 0.0f; // Never blend for breach idle animations
		}
...
//******** Find the current values for NewTargetActor and NewTargetLocation **********	
	...
	{
		// Allow the breach sequence visualizers and other functionality to do the targeting
		bFoundTarget = false;
	}
	else if( bEnemiesVisible ) //If enemies are visible but we are not in a targeting action
	{
		...
		if (LastAttacker != none && 
			...
			!LastAttacker.GetVisualizedGameState().IsIncapacitatedCapturedOrDowned() &&
		...
		else //Fall back to facing the closest enemy
		{
			...
			else if (class'X2TacticalVisibilityHelpers'.static.GetClosestVisibleEnemy(UnitNative.ObjectID, OutVisibilityInfo, -1)) //Try again, but with the latest history frame. bEnemiesVisible says we should have one...
			{
				NewTargetActor = XGUnit(History.GetVisualizer(OutVisibilityInfo.TargetID));
			}
	...
	else //In the absence of a valid target, we should choose a valid cover direction and use the cover
	{
		...
		if( UnitNative.CanUseCover() )
		{
			...
			NewTargetActor = none;
			if( BestCoverIndex > -1  && 
				((CurrentCoverPeekData.CoverDirectionInfo[BestCoverIndex].LeftPeek.bHasPeekaround != 0) || (CurrentCoverPeekData.CoverDirectionInfo[BestCoverIndex].RightPeek.bHasPeekaround != 0)) )
			{
...
//******** Ascertain whether the target/target location, or our location, has changed **********
	if( bFoundTarget )
	{
		...
		`log("			*NewTarget: " @ NewTargetActor.Name @ " OldTarget: " @ TargetActor.Name, `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
		...
		TempUnit = XGUnit(TargetActor);
		if(TempUnit != none && bActorFromTargetingMethod && TempUnit.IdleStateMachine.LastAttacker != Unit) //Skip this logic if in the breach sequence
		{   
...
native function bool NeedsToTurn(vector Position);
...
function bool ShouldFaceEntryPointMarker(optional out BreachEntryPointMarker EntryPointMarker)
{
	local int UseHistoryIndex;
	local XComGameStateHistory History;
	local XComGameState_BattleData BattleData;
	...
	BreachDataState = XComGameState_BreachData(History.GetSingleGameStateObjectForClass(class'XComGameState_BreachData'));
	BreachDataState = XComGameState_BreachData(History.GetGameStateForObjectID(BreachDataState.ObjectID, , -1)); //Always use the latest breach data for this evaluation

	//TODO TURN: We should face our entry marker until we've performed our first breach action
	foreach BreachDataState.CachedBreachPointInfos(PointInfo)
	{
		if (PointInfo.XComRefsAssignedToThisPoint.Find('ObjectID', UnitState.ObjectID) != INDEX_NONE)
		{
			foreach PointInfo.BreachSlots(SlotInfo)
			{
				if (SlotInfo.BreachUnitObjectRef.ObjectID == UnitState.ObjectID)
				{
					if (SlotInfo.BreachSlotTileLocation == UnitState.TileLocation || BattleData.IsInBreachSelection()) //Always face the entry point in breach selection
					{
						EntryPointMarker = PointInfo.EntryPointActor;
						return true;
					}

					// we have moved away from our assigned slot, no longer face the entry point
					return false;
				}
			}
			break;
	...
	foreach BreachDataState.CachedBreachPointInfos(PointInfo)
	{
		if (`XWORLD.GetTileCoordinatesFromPosition(PointInfo.BreachPointLocation) == UnitState.TileLocation)
		{
			EntryPointMarker = PointInfo.EntryPointActor;
			return true;
		}
...
state PeekStart
{
	...
	event Tick(float DeltaTime)
	{
		if (`XCOMVISUALIZATIONMGR.IsActorBeingVisualized(Unit))
		{
			GotoState('PeekStop');
		}
	}
...
state ScootOver
{
	event BeginState(name PreviousStateName)
	{		
		`log("*********************** BEGIN Scoot ************************************************", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
		`log("", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
		`log("Unit : "@Unit, `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
		`log("", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
	...
	event EndState(name NewStateName)
	{	
		`log("*********************** END Scoot ************************************************", `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
	}
...
begin:	
	AnimParams = default.AnimParams;
	AnimParams.AnimName = 'HL_FlinchPass';
...
state EvaluateStance
{
	...
	function vector GetDesiredFaceLocation()
	{
		...
		local int UseHistoryIndex;
		local BreachEntryPointMarker EntryPointMarkerToFace;
		
		UnitState = UnitNative.GetVisualizedGameState(UseHistoryIndex);
		
		//If we're in breach mode, set a specific facing based on our deploy spawn layout		
		ShouldFaceEntryPointMarker(EntryPointMarkerToFace);
		if (EntryPointMarkerToFace != None)
		{
			// Found an entry point in the same group, face it
			DesiredFaceLocation = EntryPointMarkerToFace.Location;
			if (`XWORLD.GetTileCoordinatesFromPosition(EntryPointMarkerToFace.Location) == UnitState.TileLocation)
			{
				DesiredFaceLocation = EntryPointMarkerToFace.Location + (Vector(EntryPointMarkerToFace.Rotation) * 1000.0f); // Match the facing of the entry point marker
			}
		}
	...
	{	
		if( bInLatentTurn )
		{
			UnitPawn.SetTargetLoc(UnitPawnNative.LatentTurnTarget);
		}
...
begin:	
	...
	//If we are set to bHoldCurrentPoseAsIdle, just go straight to back to idle
	`log("     bMoveFireIdle:"@ bMoveFireIdle, `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
	if (bMoveFireIdle &&
		(`PRES.GetTacticalHUD() == none || `PRES.GetTacticalHUD().GetTargetingMethod() == none)) //Only skip if the player is not in targeting
	{
		GotoState('Idle');
	}
	...
	`log("     ShouldFaceEntryPointMarker():"@(ShouldFaceEntryPointMarker()), `CHEATMGR.MatchesXComAnimUnitName(Unit.Name), 'XCom_Anim');
	if (ShouldFaceEntryPointMarker())
	{
		GotoState('Idle');
	}
			...
			{	
				...
				if( DesiredCoverIndex > -1 && !bForceTurnTarget ) //The unit needs to: turn, enter cover in the new direction
				{
					if (UnitNeedsToRotate(TempFaceLocation))
					{
						...
						StoredTurnThreshold = UnitPawn.AimingNeedsToTurnThresholdDegrees; //Hack to fix issue with units not standing properly in cover sometimes
						UnitPawn.AimingNeedsToTurnThresholdDegrees = 1;
						TurnTowardsPosition(TempFaceLocation, true);//Latent turning function on XGUnit
						UnitPawn.AimingNeedsToTurnThresholdDegrees = StoredTurnThreshold;
...
Exit:
	...
	else
	{
		...
		if (ReturnToState == 'Crouch' && !bContinueCrouching)
		{
			GotoState('Idle');
		}
		GotoState(ReturnToState);
...
{	
	...
	function SetTargetLoc()
	{
		local Vector NewTargetLoc;

		NewTargetLoc = Unit.AdjustTargetForMiss(XGUnit(TargetActor), 3.0f);
		UnitPawn.SetTargetLoc(NewTargetLoc);
	}
...
state PersistentEffect
{
	...
	function Vector GetWrathCannonTargetLoc()
	{
		...
		local int NumRoundBegins;

		History = `XCOMHISTORY;
		//initialize the location
		TargetLoc = TargetLocation;
		NumRoundBegins = 0;	//don't search the history forever
		for( ScanHistory = History.GetCurrentHistoryIndex(); ScanHistory >= 0 && NumRoundBegins < 2; --ScanHistory )
		{
			//with interleaved turns, the player turn begin can happen many times a round
			//so break the loop if ability not found and more than two rounds have passed
			...
			if( GameRuleContext != None && GameRuleContext.GameRuleType == eGameRule_RoundBegin )
			{
				NumRoundBegins += 1;
			}
...
Begin:
	...
	while( PersistentEffectIdleName != '' )
	{
		...
		{
			UnitPawnNative.SetTargetLoc(TargetLocation);
		}
		Sleep(0.0f);
	}
