...
const MinFactionForceLevel = 1;
const MaxFactionForceLevel = 20;

enum EInvestigationStage
{
	eStage_Groundwork,
	eStage_Operations,
	eStage_Takedown,
...
enum EOperationStage
{
	eOpStage_Casework,
	eOpStage_LeadActive,
	eOpStage_MissionActive,
	eOpStage_Completed,
};

enum EWorkerAlignment
{
	eWorker_Player,
	eWorker_Faction,
	eWorker_Neutral,	// All side missions and situations
};

enum EWorkerActionResponse
{
	eActionResponse_Destroy,
	eActionResponse_ReturnHome,
	eActionResponse_RemainInPlace,
};

enum EStrategyActionType
{
	eStrategyAction_Mission,
	eStrategyAction_Situation,
	eStrategyAction_CityRegion,
	eStrategyAction_HQ,
};

enum EStrategyActionAssignment
{
	eAssignment_Unassigned,		// Not assigned to any strategy action. Available for assignment
	eAssignment_Assigned,		// Assigned to a strategy action pending start
	eAssignment_Spent,			// Participated in a strategy action this round, cannot be re-used until next round
	eAssignment_Blocked,		// Cannot be assigned to any strategy action (likely due to injury)
};

enum EStrategyActionCommand
{
	eActionCommand_Start,
	eActionCommand_Cancel,
	eActionCommand_Fail,
};
...
enum EStrategyNotificationType
{
	eSNT_UnassignedUnits,
	eSNT_Promotions,
	eSNT_AssemblyComplete,
	eSNT_NormalScar,
	eSNT_LeftBehindScar,
	eSNT_AssemblyIdle,
	eSNT_TrainingComplete,
	eSNT_SpecOpsUnlocked,
	eSNT_SpecOpsComplete,
	eSNT_SupplyNewItems,
	eSNT_ScavengerMarketOpen,
	eSNT_CriticalMissionAlert,
	eSNT_PassiveIncome,
	eSNT_UnlockAgent,
	eSNT_UnrestAnarchyChange,
	eSNT_PaceWarningSpecOps,
	eSNT_PaceWarningTraining,
	eSNT_PaceWarningFieldTeams,
	eSNT_PaceWarningFieldTeamAbilities,
	// Tutorial
	eSNT_TutorialAssemblyAvailable,
	eSNT_TutorialSpecOpsAvailable,
	eSNT_TutorialTrainingAvailable,
	eSNT_TutorialNewAndroid,
	// HELIOS BEGIN
	// Add new enums for notificaitons
...
struct native CampaignStartData
{
	var name InvestigationTemplateName;
	// Relative to UITacticalQuickLaunch_MapData Soldiers
	var array<name> InitialSquadIDs;
};

struct native StrategyActionAccessConstraints
{
	var array<name> RequiredSoldierClassNames;	// SoldierClassTemplate names for any squad members that *must* be assigned to start the action
};

struct native WeightedDataName
{
	var float Weight;
	var name DataName;
...
struct native IntWeightedDataName
{
	var int Weight;
	var name DataName;
...
// Circumstances that lead to scar chance, set on a Unit during Tactical
enum EScarChanceFlags
{
	eScarFlag_HalfHP,
	eScarFlag_BleedOut,
};

enum EScarSourceFlags
{
	eScarSource_Wounded,
	eScarSource_BleedOut,
	eScarSource_LeftBehind,
};

// Tracking for changes to a scar over time
struct native ScarHistoryData
{
	var EScarSourceFlags Source;
	var int MissionObjectID;
};

// For tracking what kind of scar a unit earned in tactical
struct ScarCandidateData
{
	var StateObjectReference UnitReference;
	var EScarSourceFlags ScarSource;
};
...
enum EInjuryState
{
	...
	eInjury_Rattled,
	eInjury_Bloodied,
	eInjury_ShellShocked,
};

enum StrategyStartType
{
	eStrategyStartType_NewCampaign,
	eStrategyStartType_LoadCampaign,
	eStrategyStartType_FromTactical,
	eStrategyStartType_Tutorial,
};

struct native DioMissionDifficultyParams
{
	var int Act;		// 1-3: Current act of the Campaign
	var int Stage;		// 1-5: interpretation of the current Investigation's stage -- roughly the Investigation's internal 'force level'
	var int District;	// 0-100: interpretation of the mission district's unrest level
};

// Tracking for completed steps in an investigation for display in Debrief Screen Journal
struct native InvestigationHistoryData
{
	var name OperationTemplateName;
	var StateObjectReference OperationRef;
	var int TurnCompleted;
	var string ImageString;
};

enum EActionSequenceState
{
	eSequenceState_NotStarted,
	eSequenceState_ActionPending,
	eSequenceState_ActionLive,
	eSequenceState_Completed,
	eSequenceState_Failed,
};

// Used to init template-based rewards from data
struct native StrategyActionRewardData
{
...
// Data to configure the creation of a Strategy Action
struct native StrategyActionConfigData
{
	var name ActionClass;	// e.g. 'Situation,' 'Mission'
	var name TemplateName;
	var array<StrategyActionRewardData> Rewards;
};

// Stores turn durations with a norm-% chance to appear
struct StrategyScheduleItemDuration
{
	var int Turns;
	var float Weight;
...
struct StrategyScheduleItemRewardSet
{
	var name PrimaryRewardName;
	var StrategyActionRewardData BonusRewardData;
};

struct StrategyScheduleItemData
{
	// Config
	var name Source;
	var name SourceDeck;
	var int NumTargets;
	var array<StrategyScheduleItemRewardSet> RewardOptions;
	var array<StrategyScheduleItemDuration> DurationOptions;
...
struct native MissionPerformanceResultData
{
	var string FormattedString;
	var string ColorString;
};

// Used to init StrategyMarket inventories from templates
struct DioStrategyMarketItem
{
	var name TemplateName;
	var int CreditsCost;
	var int IntelCost;
...
struct native DioStrategyMarketCostMod
{
	var name MarketGroupID;
	var string SourceDisplaySummary;
	var int CostMod;
};

struct DioStrategyNameCollection
{
	var name CollectionName;
	var array<name> CollectionMembers;
};

struct native AndroidNameAppearance
{
	var string NickNameID;	// Matches Android NickName when chosen
	var int ArmorTint;
	var int ArmorTintSecondary;
};

enum ECampaignGoalStatus
{
	eGoalStatus_None,
	eGoalStatus_Warning,
	eGoalStatus_Failed,
...
// Moved here from UITacticalQuickLaunch_MapData so that core classes can have access [1/18/2019 dmcdonough]
struct native ConfigurableSoldier
{
	var name SoldierID;
	var bool bUseSoldierFromStartState;
...
struct native DarkEventStrategyData
{
	var name TemplateName;
	var name TacticalTag;
	var int	OverrideAppearChance;	// Optional, used for Ability-based Dark Events that don't build in their chance to appear
	var bool bValidOnTakedown;		// Optional: if set, the dark event will be considered for Takedown missions

	structdefaultproperties
	{
		OverrideAppearChance = -1;
		bValidOnTakedown = false;
	}
};
...
struct native Commodity
{
	...
	var int							IntelCost;
	var int							EleriumCost;
	var bool						bRemoveAfterPurchase;
	var String						Title;
	var String						Desc;
	var String						CostModString;	// optional
	var String						Image;

	structdefaultproperties
	{
		bRemoveAfterPurchase = false; // Default is unlimited sales
	}
};
...
struct native ObjectiveDisplayInfo
{
	...
	var init string MissionType;
	var init string DisplayLabel; // Label that appears in the objective list
	var init string DisplaySubLabel; // Subtitle / sub label that appears in the objective list ( used by timers potentially )
	var int UiState;
...
struct native LastMissionData
{
	...
	var StateObjectReference RegionRef;	//	The region where the mission occurred
...
//				DIO VARS
//---------------------------------------------------------------------------------------
var config float	ScarChances[EScarChanceFlags.EnumCount]<BoundEnum = EScarChanceFlags>;			// Chances (0-100) for a Unit to roll a scar after a mission
var config int		RequiredStageMissions[EInvestigationStage.EnumCount]<BoundEnum = EInvestigationStage>;			// Number of missions that must be completed at each Investigation Stage to advance to the next
var config int		MissionAvailableTurns[EInvestigationStage.EnumCount]<BoundEnum = EInvestigationStage>;			// Number of turns missions at each stage will be kept available

// Loc Strings
var localized string CampaignGoalStatusLabels[ECampaignGoalStatus.EnumCount]<BoundEnum = ECampaignGoalStatus>;
var localized string ActionAssignmentLabels[EStrategyActionAssignment.EnumCount]<BoundEnum = EStrategyActionAssignment>;
var localized string InvestigationStageLabels[EInvestigationStage.EnumCount]<BoundEnum = EInvestigationStage>;
var localized string InvestigationStageMissionLabels[EInvestigationStage.EnumCount]<BoundEnum = EInvestigationStage>;
...
var localized array<string> DaysOfTheWeek;
...
var config array<int> DayOfTheWeekHelperArray;
...
//************************************************************
// Strategy notification data stack 
struct native TStrategyNotificationData
{
	var EStrategyNotificationType Type;

	var string Title;
	var string Body;
	var string UIColor;
	var string ImagePath;	//for the category icon 
	var int IconSize;		// DEPRECATED bsteiner 10.28.2019

	var array<StateObjectReference>	UnitRefs;

	var delegate<OnItemSelectedCallback>    OnItemSelectedFn;
};
...
static function int GetDayOfTheWeekIndex(TDateTime kDateTime)
{
	local int y, m, d, rawDoW;

	y = kDateTime.m_iYear;
	m = kDateTime.m_iMonth;
	d = kDateTime.m_iDay;

	// Tomohiko Sakamoto's algorithm, based on standard Doomsday algorithm 
	// Results in Monday = 1 through Sunday = 7
	//In the config: 
	//DayOfTheWeekHelperArray = (0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4);
	y -= int(m < 3);
	rawDoW = (y + y / 4 - y / 100 + y / 400 + default.DayOfTheWeekHelperArray[m - 1] + d) % 7;

	return rawDoW;
}

static function string GetDayOfTheWeekString(TDateTime kDateTime)
{
	local int adjustedDoW;

	adjustedDoW	= GetDayOfTheWeekIndex(kDateTime) - 1;
	if( adjustedDoW < 0 ) adjustedDoW += default.DaysOfTheWeek.length; 
	
	if( adjustedDoW >= 0 && adjustedDoW < default.DaysOfTheWeek.length )
	{
		//offset by 1 to map to our DotW helper array. 
		return default.DaysOfTheWeek[adjustedDoW];
	}
	else
	{
		return "";
...
static function OnAddedToCrewSoldierCompatRolls(XComGameState NewGameState, XComGameState_HeadquartersXCom XComHQ, XComGameState_Unit UnitState)
{
	...
	{
		if(XComHQ.Crew[idx] != UnitState.GetReference() && XComHQ.Crew[idx].ObjectID > 0)
		{
			if (`XCOMHISTORY.GetGameStateForObjectID(XComHQ.Crew[idx].ObjectID, eReturnType_Reference) != none)
			{
