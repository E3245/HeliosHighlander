...
var() protectedwrite array<StateObjectReference> KilledUnitsLastMission, KilledUnitsLastTurn, CapturedUnits, CapturedUnitsLastTurn;
...
var() bool bBleedingOut;						// expects to be cleared during OnEndTacticalPlay
var() bool bUnconscious;						// expects to be cleared during OnEndTacticalPlay
var() bool bInStasis;
...
var() bool bTreatNoCoverAsLow;					//GetCoverTypeFromLocation will return CT_MidLevel instead of CT_None
...
var() bool bSpawnedFromAvenger;					// unit was spawned from the avenger for a defense mission // expects to be cleared during OnEndTacticalPlay
...
var() bool bLifelined;							// unit has triggered its lifeline, but has not resolved to the final state (waiting on ability triggers to resolve). // expects to be cleared during OnEndTacticalPlay
var() bool bBreachAssigned;						// units has been assigned (or unassigned/not-yet-assigned) to a breach point, sometimes a deploy-spawn and entry-point coexist on a tile, we must track actual assignment now // expects to be cleared during OnEndTacticalPlay
...
var StateObjectReference CopyUnitAppearance;	// Store the state of the unit that X2Effect_SpawnUnit would like THIS unit to use
...
var int AssignedToRoomID;				//DIO: This is set at spawn and used to identify which room the character occupies
var name BreachTargetingAbilityName;	//DIO: During breach targeting, what is the exclusive ability being targeted by this unit
var bool bCanTraverseRooms;				

var name AssignedVoiceID; 
var bool bForceValidSpeaker;			// if ture, will bypass CheckUnitValidForVO. gets cleaned up in TacticalBegin & TacticalEnd
...
//---------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------
var() EStrategyActionAssignment		HQAssignment;
var() array<StateObjectReference>	Scars;
var() name							ScarsDeck;
var() int							LastRoundScarAdded;	// tracks the last strategy round this unit gained a new Scar
var() privatewrite array<EScarChanceFlags>	ActiveScarFlags;	// Set during tactical missions, used/cleared when returning to Strategy

var() privatewrite array<name> Specialties;

var() array<name> AvailableTrainingPrograms;
var() array<name> CompletedTrainingPrograms;
var() array<StateObjectReference> UnseenCompletedTrainingActions;
var() array<StateObjectReference> UnseenCompletedSpecOpsActions;
...
// bsg-dforrest (4.10.17): transient flag for preloading voice datat with RequestResources
var private transient bool bLoadVoiceWhenRequestResources;
// bsg-dforrest (4.10.17): end
...
// mmg-svasilyev (12.13.17) : PreviewTargets HUD icon update optimization
struct native PreviewTargetsState
{
	var bool bCurrentlyAffectable;
	var bool bSSEnemy;
	var bool bFlankedTarget;
};

var PreviewTargetsState m_PreviewTargetsState;
// mmg-svasilyev (12.13.17) : end

delegate OnUnitPawnCreated( XComGameState_Unit Unit);

// Call to set the tactical transient flag for this unit, marking it for removal at the end of a tactical mission
native function MakeTacticalTransient();
...
event SetVisibilityLocation(const out TTile VisibilityTile)
{
	...
	local bool NeedsToCheckVolumeTouches, NeedsToCheckExitTouches, NeedsToCheckRoomVolumeTouches;
	...
	local XComGameState_EvacZone AlienEvacZoneState;
	...
	if( TileLocation != VisibilityTile )
	{
		...
		{
			`redscreen( "SetVisibilityLocation given a tile that needed to be clamped to a valid location.\n Given: " $ VisibilityTile.X @ VisibilityTile.Y @ VisibilityTile.Z @ "Clamped:" $ Clamped.X @ Clamped.Y @ Clamped.Z @ "WorldNumTiles:" $ WorldData.NumX @ WorldData.NumY @ WorldData.NumZ @ "\nUnit ID:" $ ObjectID @ self.GetMyTemplateName() @ "\nTrace:\n" @ GetScriptTrace() @ "\n" );
		}
		...
		NeedsToCheckRoomVolumeTouches = !BattleData.IsInBreachSelection();
		if (NeedsToCheckRoomVolumeTouches)
		{
			NewRoomID = WorldData.GetRoomIDByTileLocationNative(Clamped);
			if (NewRoomID != AssignedToRoomID)
			{
				AlienEvacZoneState = class'XComGameState_EvacZone'.static.GetEvacZone(eTeam_Alien);
				EffectState = GetUnitAffectedByEffectState(class'X2Effect_FleeToNextRoom'.default.EffectName);
				if (AlienEvacZoneState == None && EffectState != none && EffectState.AffectedTiles.length > 0 )
				{
					EventManager.TriggerEvent('UnitFled', self, self);
				}
				NewRoomID = WorldData.GetRoomIDByTileLocationNative(TileLocation);
				AssignedToRoomID = NewRoomID;
				EventManager.TriggerEvent('UnitEnteredNewRoom', self);
			}
		}
	...
	if( InOutVisibilityInfo.bVisibleBasic )
	{
		...
		else
		{
			...
			if(kTargetCurrent != none)
			{
				...
				{
					// AI + Player units shouldn't be able to see each other during breach selection phase
					BattleData = XComGameState_BattleData(History.GetSingleGameStateObjectForClass(class'XComGameState_BattleData'));
					if (IsEnemyUnit(kTargetCurrent) 
						&& (BattleData.IsInBreachSelection() == true || !HasAssignedRoomBeenBreached() || !kTargetCurrent.HasAssignedRoomBeenBreached()))
					{
						InOutVisibilityInfo.bVisibleGameplay = false;
						InOutVisibilityInfo.GameplayVisibleTags.AddItem('BreachConcealment');
					}
...
function XComGameState_Unit GetUnit(optional XComGameState ModifyGameState=none)
{
	if (ModifyGameState != none)
	{
		return XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
	}
...
function string GetCoverStatusLabel()
{
	local string CoverStatus;
	local ECoverType CoverType;

	CoverStatus = "_none";
	CoverType = GetCoverTypeFromLocation();
	if( CoverType != CT_None )
	{
		if( CoverType == CT_Standing )
		{
			CoverStatus = "_highCover";
		}
		else
		{
			CoverStatus = "_lowCover";
		}
	}
...
//Generates a single GFxObject that contains all usefull information about a unit
//that Flash would need. Allows for passing a single object to ActionScript.
function GFxObject MakeBasicInfoObjectForMovie(UIMovie Movie, optional int selectedID = 0)
{
	local GFxObject gfx;
	local X2SoldierClassTemplate SoldierClassTemplate;
	local string ClassName, RankName;
	local Texture2D StaffPicture;

	SoldierClassTemplate = GetSoldierClassTemplate();
	ClassName = SoldierClassTemplate.DisplayName;
	RankName = class'X2ExperienceConfig'.static.GetRankName(GetRank(), SoldierClassTemplate.DataName);
	StaffPicture = `XENGINE.m_kPhotoManager.GetHeadshotTexture(1, ObjectID, 512, 512);

	gfx = Movie.CreateObject("Object");
	gfx.SetFloat("objectID", ObjectID);
	gfx.SetString("soldierName", GetName(eNameType_Full));
	gfx.SetString("soldierNick", GetNickName());
	gfx.SetString("soldierClassName", ClassName);
	gfx.SetString("soldierRankName", RankName);
	gfx.SetString("soldierRank", class'UIUtilities_Image'.static.GetRankIcon(GetRank(), GetSoldierClassTemplateName()));
	gfx.SetString("soldierClass", SoldierClassTemplate.IconImage);
	gfx.SetString("soldierProfile", SoldierClassTemplate.ProfileIconImage);
	gfx.SetString("SoldierImage", class'UIUtilities_Image'.static.ValidateImagePath(PathName(StaffPicture)));

	return gfx;
}

event UpdateObjectForMovie(UIMovie Movie, GFxObject UpdateObj, optional int selectedID = 0)
{
	local XComGameStateHistory History;
	local XComGameState_Player LocalPlayerObject;
	local GFxObject gfxPenalties, gfxBonuses, gfxPassives, gfxStatusIcons;
	local string CoverStatus;// , breachStatus;
	local bool bShowBonus, bShowPenalty, bOnlyVisibleEnemies, bIsFlanked, bIsFriendly, bIsSelected, bHasAmmo, bShowPassives; // , bIsBreachReady;
	local int VisualizedHistoryIndex, i, CurrentAmmo, MaxAmmo, AmmoCost;
	local XGUnit UnitVisualizer;
	local PlayerController PC;
	local array<UISummary_UnitEffect> BonusEffects, PenaltyEffects, PassiveEffects;
	local array<string> StatusIcons;
	local XComGameState_Item PrimaryWeapon, LoadedAmmo;
	...
	local int HistoryFrame;
	local int LocalPreviousHistoryFrame;
	local bool bIsTactical;
	// HELIOS VALUES
	local XComGameState_Effect_TemplarFocus FocusState;

	//Perform a minimal rebuild / update of the GFx visualizer object
	History = `XCOMHISTORY;

	//First, see if any new history frames have come in since we last updated. If not we can skip this entirely.
	HistoryFrame = int(UpdateObj.GetFloat("GlobalHistoryFrame"));
	if (HistoryFrame == History.GetCurrentHistoryIndex())
	{
		return;
	}

	//The global history frame has changed, update us
	UpdateObj.SetFloat("GlobalHistoryFrame", float(History.GetCurrentHistoryIndex()));

	//New history frames have come in since our last update. The next major filter is what game mode we are in.
	bIsTactical = `TACTICALGRI != none;
	if (bIsTactical)
	{
		//These fields can only change in tactical
		//========================================

		//Further filter based on whether THIS object has changed
		LocalPreviousHistoryFrame = int(UpdateObj.GetFloat("LocalPreviousHistoryFrame"));
		if (LocalPreviousHistoryFrame != PreviousHistoryFrameIndex)
		{
			//This object has been updated since the last update. Update any fields that can only change when this object changes state
			//=========================================================================================================================
			
			//Record the new previous history frame index
			UpdateObj.SetFloat("LocalPreviousHistoryFrame", float(PreviousHistoryFrameIndex));

			//May change due to mind control
			UnitVisualizer = XGUnit(GetVisualizer());
			PC = class'Engine'.static.GetCurrentWorldInfo().GetALocalPlayerController();
			bIsFriendly = UnitVisualizer.IsFriendly(PC);
			UpdateObj.SetBool("isFriendly", bIsFriendly);

			//May change if the unit moves
			UpdateObj.SetFloat("movesLeft", NumAllActionPoints());

			//May change during ability effect application
			UpdateObj.SetFloat("currentHP", GetCurrentStat(eStat_HP));
			UpdateObj.SetFloat("maxHP", GetMaxStat(eStat_HP));
			UpdateObj.SetFloat("currentShield", GetCurrentStat(eStat_ShieldHP));
			UpdateObj.SetFloat("maxShield", GetMaxStat(eStat_ShieldHP));
			UpdateObj.SetFloat("currentArmor", GetArmorMitigationForUnitFlag());
			UpdateObj.SetFloat("maxArmor", GetMaxArmor());
			UpdateObj.SetFloat("shreddedArmor", Shredded);

			//May change when firing a weapon or getting an abliity effect applied
			bHasAmmo = false;
			PrimaryWeapon = GetPrimaryWeapon();
			if (PrimaryWeapon != none && PrimaryWeapon.ShouldDisplayWeaponAndAmmo())
			{
				bHasAmmo = true;
				if (PrimaryWeapon.HasInfiniteAmmo())
				{
					CurrentAmmo = 1;
					MaxAmmo = 1;
					AmmoCost = 0;
				...
				{
					CurrentAmmo = PrimaryWeapon.Ammo;
					MaxAmmo = PrimaryWeapon.GetClipSize();
					AmmoCost = 0; // TODO: GetPotentialAmmoCost()

					if (PrimaryWeapon.LoadedAmmo.ObjectID != 0)
					{
						LoadedAmmo = XComGameState_Item(History.GetGameStateForObjectID(PrimaryWeapon.LoadedAmmo.ObjectID));
						AmmoTemplate = X2AmmoTemplate(LoadedAmmo.GetMyTemplate());
						if (AmmoTemplate != None)
						{
							UpdateObj.SetString("ammoImage", AmmoTemplate.strBackpackIcon);
						}
					}
				}
			}
			UpdateObj.SetBool("hasAmmo", bHasAmmo);
			UpdateObj.SetFloat("currentAmmo", CurrentAmmo);
			UpdateObj.SetFloat("maxAmmo", MaxAmmo);
			UpdateObj.SetFloat("ammoCost", AmmoCost);

			UpdateObj.SetString("scarsImage", Scars.length > 0 ? class'UIUtilities_Image'.const.ScarIcon : "");
			
			//These are expensive enough that it might be worth filtering them additionally by analyzing stored
			//state of the prior history frame to see if anything within these arrays actually changed
			gfxPenalties = Movie.CreateArray();
			PenaltyEffects = GetUISummary_UnitEffectsByCategory(ePerkBuff_Penalty);
			for (i = 0; i < PenaltyEffects.length; i++)
			{
				gfxPenalties.SetElementObject(i, MakeEffectObjectForMovie(Movie, PenaltyEffects[i]));
			}
			UpdateObj.SetObject("penalties", gfxPenalties);
			bShowPenalty = (PenaltyEffects.length > 0);
			UpdateObj.SetBool("showPenalty", bShowPenalty);

			gfxBonuses = Movie.CreateArray();
			BonusEffects = GetUISummary_UnitEffectsByCategory(ePerkBuff_Bonus);
			for (i = 0; i < BonusEffects.length; i++)
			{
				gfxBonuses.SetElementObject(i, MakeEffectObjectForMovie(Movie, BonusEffects[i]));
			}
			UpdateObj.SetObject("bonuses", gfxBonuses);
			bShowBonus = (BonusEffects.length > 0);
			UpdateObj.SetBool("showBonus", bShowBonus);

			gfxPassives = Movie.CreateArray();
			PassiveEffects = GetUISummary_UnitEffectsByCategory(ePerkBuff_Passive);
			for (i = 0; i < PassiveEffects.length; i++)
			{
				gfxPassives.SetElementObject(i, MakeEffectObjectForMovie(Movie, PassiveEffects[i]));
			}
			UpdateObj.SetObject("passives", gfxPassives);
			bShowPassives = (PassiveEffects.length > 0);
			UpdateObj.SetBool("showPassives", bShowPassives);

			gfxStatusIcons = Movie.CreateArray();
			StatusIcons = GetUISummary_UnitStatusIcons();
			for (i = 0; i < StatusIcons.length; i++)
			{
				gfxStatusIcons.SetElementString(i, StatusIcons[i]);
			}
			UpdateObj.SetObject("statusIcons", gfxStatusIcons);

			//May change during breach
			//DeploymentSpawn = class'XComBreachHelpers'.static.GetUnitDeploymentSpawn(self);
			//bIsBreachReady = DeploymentSpawn != none;			
			//SlotInfo = class'XComBreachHelpers'.static.GetUnitBreachSlotInfo(ObjectID);
			//breachStatus = bIsBreachReady ? class'XLocalizedData'.default.BreachReadyForBreach : class'XLocalizedData'.default.BreachChoosePosition;
			//bIsBreachReady = DeploymentSpawn != none;
			//UpdateObj.SetBool("isBreachReady", bIsBreachReady);
			//UpdateObj.SetFloat("groupId", bIsBreachReady ? DeploymentSpawn.BreachGroupID : -1);
			//UpdateObj.SetObject("breachAbilities", MakeBreachAbilityObjectForMovie(Movie));
			//UpdateObj.SetString("breachStatus", breachStatus);			
			//UpdateObj.SetFloat("breachMovementOrder", bIsBreachReady ? SlotInfo.BreachSlotIndex : -1);

			if (GetSoldierClassTemplateName() == 'SoldierClass_Breaker')
			{
				UpdateObj.SetString("specialIcon", class'UIUtilities_Image'.const.SoldierBreaker_Focus);
				UpdateObj.SetFloat("specialCount", float(GetTemplarFocusLevel()));
			}
			else if (GetSoldierClassTemplateName() == 'SoldierClass_Warden')
			{
				UpdateObj.SetString("specialIcon", class'UIUtilities_Image'.const.SoldierWarden_Focus);
				UpdateObj.SetFloat("specialCount", float(GetTemplarFocusLevel()));
			}
			else if(GetSoldierClassTemplateName() == 'SoldierClass_Envoy')
			{
				ObjectRef.ObjectID = ObjectID;
				//Psionic network ability - can optimize out by checking whether the character has this or not potentially
				VisualizedHistoryIndex = `XCOMVISUALIZATIONMGR.LastStateHistoryVisualized;
				class'X2Effect_NeuralNetwork'.static.GatherEnemiesInNeuralNetwork(ObjectRef, NetworkedEnemyRefs, VisualizedHistoryIndex);
				UpdateObj.SetString("specialIcon", class'UIUtilities_Image'.const.SoldierEnvoy_Network);
				UpdateObj.SetFloat("specialCount", NetworkedEnemyRefs.Length);
			}
			//HELIOS BEGIN
			// Use the Unit State's FocusState to Display custom Icons and stats
			else
			{
				// Attempt to grab an existing State
				FocusState = GetTemplarFocusEffectState();

				if (FocusState != none)
				{
					UpdateObj.SetString("specialIcon", FocusState.GetImageForGFX(self));
					UpdateObj.SetFloat("specialCount", FocusState.GetValueForGFX(self));
				}
			}
			// HELIOS END
		}

		//Now update any fields that might change at any time due to any history change...
		//================================================================================
		
		//May change if ANY unit moves position		
		CoverStatus = GetCoverStatusLabel();
		LocalPlayerObject = XComGameState_Player(`XCOMHISTORY.GetGameStateForObjectID(`TACTICALRULES.GetLocalClientPlayerObjectID()));
		bOnlyVisibleEnemies = !IsEnemyTeam(LocalPlayerObject.GetTeam());
		bIsFlanked = IsFlanked(, bOnlyVisibleEnemies, VisualizedHistoryIndex) && !(ControllingPlayerIsAI() && GetCurrentStat(eStat_AlertLevel) == 0);
		UpdateObj.SetBool("isFlanked", bIsFlanked);
		UpdateObj.SetString("coverStatus", CoverStatus);
		UpdateObj.SetFloat("coverValue", float(GetCoverTypeFromLocation()));
		UpdateObj.SetFloat("coverIndex", GetCoverTypeFromLocation());
		UpdateObj.SetFloat("previewCoverDamage", 0);

		//May change due to user input actions.  UNRELIABLE!
		bIsSelected = selectedID == ObjectID;
		UpdateObj.SetBool("isSelected", bIsSelected);
		UpdateObj.SetFloat("aimPercent", GetCurrentStat(eStat_Offense));
	}
	else
	{
		//These fields can only change in strategy
		UpdateObj.SetString("soldierRank", class'UIUtilities_Image'.static.GetRankIcon(GetRank(), GetSoldierClassTemplateName()));
	}
}

event GFxObject CreateObjectForMovie(UIMovie Movie, optional int selectedID = 0)
{
	return MakeObjectForMovie(Movie, selectedID);
}

//Generates a single GFxObject that contains all usefull information about a unit
//that Flash would need. Allows for passing a single object to ActionScript.
function GFxObject MakeObjectForMovie(UIMovie Movie, optional int selectedID = 0)
{
	local GFxObject gfx, gfxPenalties, gfxBonuses, gfxPassives;
	local X2SoldierClassTemplate SoldierClassTemplate;
	local XComGameState_Player LocalPlayerObject;
	local string ClassName, RankName, CoverStatus; // , breachStatus;
	local bool bShowBonus, bShowPenalty, bOnlyVisibleEnemies, bIsFlanked, bIsFriendly, bIsSelected, bHasAmmo, bShowPassives; // , bIsBreachReady;
	local int VisualizedHistoryIndex, i, CurrentAmmo, MaxAmmo, AmmoCost;
	local XGUnit UnitVisualizer;
	local PlayerController PC;
	local array<UISummary_UnitEffect> BonusEffects, PenaltyEffects, PassiveEffects;
	//local XComDeploymentSpawn DeploymentSpawn;
	local Texture2D StaffPicture;
	...
	gfx = Movie.CreateObject("Object", , false);
	gfx.SetFloat("objectID", ObjectID);
	gfx.SetString("soldierName", GetName(eNameType_Full));
	gfx.SetString("soldierNameCaps", `MAKECAPS(GetName(eNameType_Full)));
	gfx.SetString("soldierNick", GetNickName());
	gfx.SetString("soldierClassName", ClassName);
	gfx.SetString("soldierRankName", RankName);
	gfx.SetString("statusText", ""); // /**/
	gfx.SetString("soldierRank", class'UIUtilities_Image'.static.GetRankIcon(GetRank(), GetSoldierClassTemplateName()));
	gfx.SetString("soldierClass", SoldierClassTemplate.IconImage);
	gfx.SetString("soldierProfile", SoldierClassTemplate.ProfileIconImage);
	gfx.SetString("soldierProfilePosed", SoldierClassTemplate.PosedIconImage);
	gfx.SetBool("isPlayer", SoldierClassTemplate != none);
	gfx.SetString("soldierWorkerIcon", GetWorkerIcon());
	gfx.SetString("factionIcon", class'UIUtilities_Image'.static.GetFactionIcon(EFaction(GetCharacterTemplate().UnitFaction)));

	gfx.SetBool("isLeader", false);
	gfx.SetBool("isLeveledUp", CanRankUpSoldier());
	gfx.SetBool("isFlanked", bIsFlanked);
	gfx.SetBool("isFriendly", bIsFriendly);
	gfx.SetBool("isSelected", bIsSelected);
	gfx.SetFloat("aimPercent", GetCurrentStat(eStat_Offense));
	gfx.SetBool("showBonus", bShowBonus);
	gfx.SetBool("showPenalty", bShowPenalty);
	gfx.SetBool("showPassives", bShowPassives);
	gfx.SetFloat("movesLeft", NumAllActionPoints());
	gfx.SetString("coverStatus", CoverStatus);
	gfx.SetFloat("coverValue", float(GetCoverTypeFromLocation()));
	gfx.SetFloat("currentHP", GetCurrentStat(eStat_HP));
	gfx.SetFloat("maxHP", GetMaxStat(eStat_HP));
	gfx.SetFloat("currentShield", GetCurrentStat(eStat_ShieldHP));
	gfx.SetFloat("maxShield", GetMaxStat(eStat_ShieldHP));
	gfx.SetFloat("currentArmor", GetArmorMitigationForUnitFlag());
	gfx.SetFloat("maxArmor", GetMaxArmor());
	gfx.SetFloat("shreddedArmor", Shredded);
	gfx.SetObject("penalties", gfxPenalties);
	gfx.SetObject("bonuses", gfxBonuses);
	gfx.SetObject("passives", gfxPassives);
	
	gfx.SetString("specialIcon", "");
	gfx.SetFloat("specialCount", NetworkedEnemyRefs.Length);

	gfx.SetFloat("coverIndex", GetCoverTypeFromLocation());
	gfx.SetFloat("previewCoverDamage", 0);

	gfx.SetBool("hasAmmo", bHasAmmo);
	gfx.SetFloat("currentAmmo", CurrentAmmo);
	gfx.SetFloat("maxAmmo", MaxAmmo);
	gfx.SetFloat("ammoCost", AmmoCost);

	gfx.SetObject("weaponImages", MakePrimaryWeaponImagesForMovie(Movie));
	gfx.SetString("SoldierImage", class'UIUtilities_Image'.static.ValidateImagePath(PathName(StaffPicture)));
	...
	//bIsBreachReady = DeploymentSpawn != none;
	//breachStatus = bIsBreachReady ? class'XLocalizedData'.default.BreachReadyForBreach : class'XLocalizedData'.default.BreachChoosePosition;
	//gfx.SetBool("isBreachReady", bIsBreachReady);
	//gfx.SetFloat("groupId", bIsBreachReady ? DeploymentSpawn.BreachGroupID : -1);
	//gfx.SetObject("breachAbilities", MakeBreachAbilityObjectForMovie(Movie));
	//gfx.SetString("breachStatus", breachStatus);
	//gfx.SetFloat("breachScore", 0); //deprecated
	//gfx.SetFloat("breachMovementOrder", bIsBreachReady ? SlotInfo.BreachSlotIndex : -1);

	return gfx;
}

//Generates a single GFxObject that contains all usefull information about a unit
//that Flash would need for the breach. Allows for passing a single object to ActionScript.
function GFxObject MakeBreachUnitDataForMovie(UIMovie Movie)
{
	local GFxObject gfx;
	...
	gfx.SetFloat("groupID", class'XComBreachHelpers'.static.GetUnitBreachGroupID(self));
...
public function string GetWorkerIcon()
{
	local string WorkerIcon;
	local X2SoldierClassTemplate SoldierClassTemplate;
	local X2CharacterTemplate CharacterTemplate;

	SoldierClassTemplate = GetSoldierClassTemplate();
	CharacterTemplate = GetCharacterTemplate();

	WorkerIcon = "";
	if( SoldierClassTemplate != none && SoldierClassTemplate.WorkerIconImage != "" && GhostSourceUnit.ObjectID <= 0 )
	{
		WorkerIcon = SoldierClassTemplate.WorkerIconImage;
	}
	else if( CharacterTemplate != none && CharacterTemplate.strWorkerIconImage != "" )
	{
		WorkerIcon = CharacterTemplate.strWorkerIconImage;
	}
	return WorkerIcon;
}

private function GFxObject MakePrimaryWeaponImagesForMovie(UIMovie Movie)
{
	local array<string> Images;
	local GFxObject gfxImages;
	local int i;

	gfxImages = Movie.CreateArray();

	Images = GetPrimaryWeapon().GetWeaponPanelImages();
	for (i = 0; i < Images.length; i++)
	{
		gfxImages.SetElementString(i, Images[i]);
	}

	return gfxImages;
}

private function GFxObject MakeBreachAbilityObjectForMovie(UIMovie Movie)
{
	local GFxObject gfxAbilities, gfxTemp;
	local int i;
	local array<name> AllowedTemplateNames;
	local name ActionTemplateName, DefaultActionName;
	local X2UnitActionTemplate ActionTemplate;
	local XComDeploymentSpawn DeploySpawn;
	local bool isSelected;
	
	i = 0;

	gfxAbilities = Movie.CreateArray();

	DeploySpawn = class'XComBreachHelpers'.static.GetUnitDeploymentSpawn(self);
	AllowedTemplateNames.Length = 0;
	DefaultActionName = class'XComBreachHelpers'.static.GetDefaultActionNameForDeploymentSpawn(DeploySpawn);
	if (DeploySpawn != None)
	{
		class'XComBreachHelpers'.static.GetAllowableBreachActionTemplateNames(DeploySpawn, DefaultActionName, ObjectID, AllowedTemplateNames);
	}
	else
	{
		class'XComBreachHelpers'.static.GetAllPotentialBreachActionTemplateNames(ObjectID, AllowedTemplateNames);
	}

	foreach AllowedTemplateNames(ActionTemplateName)
	{
		ActionTemplate = X2UnitActionTemplate(class'X2TacticalElementTemplateManager'.static.GetTacticalElementTemplateManager().FindTacticalElementTemplate(ActionTemplateName));
		if (ActionTemplate != None)
		{
			isSelected = DeploySpawn != None && DeploySpawn.SelectedBreachActionTemplateName == ActionTemplate.DataName;

			gfxTemp = Movie.CreateObject("Object", , false);
			gfxTemp.SetFloat("objectID", ObjectID);
			gfxTemp.SetString("dataName", string(ActionTemplate.DataName));
			gfxTemp.SetString("name", ActionTemplate.LocFriendlyName);
			gfxTemp.SetString("icon", ActionTemplate.IconSource);
			gfxTemp.SetBool("isSelected", isSelected);
			gfxAbilities.SetElementObject(i, gfxTemp);
			i++;
		}
	}
...
private function GFxObject MakeEffectObjectForMovie(UIMovie Movie, UISummary_UnitEffect effect)
{
	local GFxObject gfxTemp;

	gfxTemp = Movie.CreateObject("Object", , false);
	gfxTemp.SetString("name", effect.Name);
	gfxTemp.SetString("description", effect.Description);
	gfxTemp.SetString("icon", effect.Icon);
	gfxTemp.SetFloat("cooldown", effect.Cooldown);
	gfxTemp.SetFloat("charges", effect.Charges);
...
event int GetMyAIPlayersPriorityTargetID()
{
	local XGUnit UnitVisualizer;
	...
	{
		UnitVisualizer = XGUnit( GetVisualizer() );
		if( UnitVisualizer != None && UnitVisualizer.m_kBehavior != None && UnitVisualizer.m_kBehavior.m_kPlayer != None )
		{
			AIPlayerData = XComGameState_AIPlayerData( `XCOMHISTORY.GetGameStateForObjectID( UnitVisualizer.m_kBehavior.m_kPlayer.GetAIDataID() ) );
			if( AIPlayerData != None )
			{
				return AIPlayerData.PriorityTarget.ObjectID;
			}
		}
	}
	return 0;
}
event bool CanTargetPriorityTargets()
{
	...
	//if you're in overwatch, feel free to shoot priority targets that move within your range
	if( ReserveActionPoints.Length > 0 )
	{
		return true;
	}
	UnitVisualizer = XGUnit(GetVisualizer());
	...
	{
		DataID = UnitVisualizer.m_kBehavior.GetAIUnitDataID(ObjectID);
		if( DataID > 0 )
		{
			...
			if( AIUnitData.JobIndex != INDEX_NONE )
			{
				...
				return ( JobName == 'Hunter' || JobName == 'HostageTaker' );
			}
		}
	}
...
event bool ShouldTreatNoCoverAsLowCover()
{
	return bTreatNoCoverAsLow;
}
...
event EForceVisibilitySetting ForceModelVisible()
{
	...
	if (`TACTICALRULES.bEnableBreachMode)
	{
		if (GetTeam() != eTeam_XCom)
		{
			if ((HasAssignedRoomBeenBreached() == false && !IsFleeing()) || BattleData.IsInBreachSelection() || BattleData.BreachingRoomListIndex < 0)
			{
				return eForceNotVisible; //Enemies in other rooms are invisible, also pre-breach
			}
		}
		else
		{
			//Player units during breach setup are visible/invisible based upon breach assignment
			if (BattleData.IsInBreachSelection() && !GetMyTemplate().bIsCosmetic && class'XComBreachHelpers'.static.GetUnitDeploymentSpawn(self) == None)
			{
				return eForceNotVisible; // Units are invisible until assigned to breach slots
			}
			else if (BattleData.IsInBreachSelection() && GetMyTemplate().bIsCosmetic)
			{
				// Find owner of cosmetic unit and observe its slot assignment rules
				GetUnitValue('OwnerStateID', OwnerIDValue);
				OwnerUnitState = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(OwnerIDValue.fValue));
				if (OwnerUnitState != None && class'XComBreachHelpers'.static.GetUnitDeploymentSpawn(OwnerUnitState) == None)
				{
					return eForceNotVisible; // Cosmetic units should observe breach-slot visibility of their owner
				}
			}
			else if (BattleData.BreachingRoomListIndex < 0)
			{
				return eForceNotVisible; // If we haven't entered breach phase yet, disallow visibility
			}
		}
	}

	//Force bodies to be visible
	if (!IsAlive() || IsBleedingOut() || IsStasisLanced() || IsUnconscious() || IsCaptured() || IsDowned())
	{
		return eForceVisible;
	}

	//If our controlling player is local	
	if (UnitVisualizer.ForceVisibility != eForceNotVisible && ControllingPlayer.ObjectID == `TACTICALRULES.GetLocalClientPlayerObjectID())
	{
		return eForceVisible;
	}
...
native function AddToUnitFloatValue(name nmValue, const float fValue, optional EUnitValueCleanup eCleanup=eCleanup_BeginTurn);
...
function X2CharacterTemplate GetCharacterTemplate()
{
	...
	if( History.GetStartState() == none )
	{
		...
		return ( TemplateManager.FindCharacterTemplate(kGameStateUnit.GetMyTemplateName()) );
	}
...
function PostCreateInit(XComGameState NewGameState, X2CharacterTemplate kTemplate, int iPlayerID, TTile UnitTile, bool bEnemyTeam, bool bUsingStartState, bool PerformAIUpdate, optional const XComGameState_Unit ReanimatedFromUnit = None, optional string CharacterPoolName, optional bool bDisableScamper, optional bool bCopyReanimatedFromUnit, optional bool bCopyReanimatedStatsFromUnit)
{
	...
	// don't retain copies of enemy units throughout the campaign, discard them when the mission completes
	if (bEnemyTeam)
	{
		MakeTacticalTransient();		
	}
	...
	if (ReanimatedFromUnit != none && bCopyReanimatedFromUnit)
	{
		if (ReanimatedFromUnit.IsSoldier())
		{
			...
			CompletedTrainingPrograms = ReanimatedFromUnit.CompletedTrainingPrograms;
		}
		else
		{
	...
	{
		bHasBeenBreached = HasAssignedRoomBeenBreached();
		if( `TACTICALMISSIONMGR.ActiveMission.AliensAlerted || bHasBeenBreached)
		{
			SetCurrentStat(eStat_AlertLevel, bHasBeenBreached ? `ALERT_LEVEL_RED : `ALERT_LEVEL_YELLOW);
		}
	}
...
function bool AutoRunBehaviorTree(Name OverrideNode = '', int RunCount = 1, int HistoryIndex = -1, bool bOverrideScamper = false, bool bInitFromPlayerEachRun=false, bool bInitiatedFromEffect=false)
{
	...
	if( !IsDead() && !IsIncapacitatedCapturedOrDowned() )
	{
...
function EventListenerReturn OnPlayerTurnBegun(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	if(!`DIO_DISABLE_TURN_START_KNOWLEDGE_DUMP && ControllingPlayer.ObjectID == XComGameState_Player(EventSource).ObjectID)
	{
		History = `XCOMHISTORY;
...
function Actor FindOrCreateVisualizer( optional XComGameState Gamestate = none )
{
	...
	if(!bRemovedFromPlay && (UnitVisualizer == none || m_bSubsystem))
	{
		...
		if (IsSoldier())
		{
			if (GetItemInSlot(eInvSlot_Armor, GameState) != none)
			{
			...
			// always tick xcom units, mostly for facefx
			if (UnitVisualizer.GetPawn().Mesh != None)
			{
				UnitVisualizer.GetPawn().Mesh.bUpdateSkelWhenNotRendered = true;
			}
		}

		if (!m_CharTemplate.bIsCosmetic && m_CharTemplate.bDisplayUIUnitFlag && `PRES.m_kUnitFlagManager != none)
		{
...
{
	local XComGameState_Unit UseUnitState;
	local XGUnit UnitVisualizer;
	...
	{
		UseUnitState = XComGameState_Unit(GameState.GetGameStateForObjectID(ObjectID));
		if (UseUnitState == none)
		{
			UseUnitState = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(ObjectID));
		}
	}
	...
	SourcePawn.GameStateResetVisualizer(UseUnitState);

	//Units also set up visualizers for their items, since the items are dependent on the unit visualizer
	UnitVisualizer.ApplyLoadoutFromGameState(UseUnitState, GameState);	
	...
	UnitVisualizer.VisualizedAlertLevel = UnitVisualizer.GetAlertLevel(UseUnitState);
	...
	if ((RescueRingRadius > 0) && (GetTeam() == eTeam_Neutral) && !bRemovedFromPlay && !IsDead())
	{
	...
	if (bGeneratesCover)
	{
		class'X2Effect_GenerateCover'.static.UpdateWorldCoverDataOnSync(UseUnitState);
	}
}
...
function RegisterForEvents()
{
	...
	EventManager.RegisterForEvent(ThisObj, 'UnitTurnEnded', OnUnitTurnEnded, ELD_OnStateSubmitted, , ThisObj);
	EventManager.RegisterForEvent(ThisObj, 'UnitTurnBegun', OnUnitTurnBegun, ELD_OnStateSubmitted, , ThisObj);
	EventManager.RegisterForEvent(ThisObj, 'UnitUnconscious', OnUnitUnconscious, ELD_OnStateSubmitted, , ThisObj);
}
...
function OnBeginTacticalPlay(XComGameState NewGameState)
{
	...
	// Always start tactical missions with max health
	SetCurrentStat(eStat_HP, GetMaxStat(eStat_HP));
	...
	if (bRemovedFromPlay && !GetMyTemplate().bDontClearRemovedFromPlay)
	{
		...
		// android reserves start removed from play, as they will be added to play later if the player chooses to use them during breach
		if(!BattleDataState.DirectTransferInfo.IsDirectMissionTransfer 
			&& BattleDataState.StartingAndroidReserves.Find('ObjectID', self.ObjectID) == INDEX_NONE
			&& BattleDataState.BackupUnitRefs.Find('ObjectID', self.ObjectID) == INDEX_NONE)
		{
	...
	bRequiresVisibilityUpdate = true;
	bForceValidSpeaker = false;

	KilledUnitsLastMission.length = 0;

	// HELIOS BEGIN
	// Track captures same as killed units for XP gain
	CapturedUnits.Length = 0;
	// HELIOS END
}

function EventListenerReturn OnUnitTurnEnded(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	SetUnitFloatValue( 'HasActed', 1.0f, eCleanup_BeginTactical );

	return ELR_NoInterrupt;
}

function EventListenerReturn OnUnitTurnBegun(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	KilledUnitsLastTurn.Length = 0;
	CapturedUnitsLastTurn.Length = 0;
...
function EventListenerReturn OnUnitUnconscious(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	Captor.CapturedUnitsLastTurn.AddItem(GetReference());
	Captor.CapturedUnits.AddItem(GetReference());
	return ELR_NoInterrupt;
}

function ApplyFirstTimeStatModifiers()
{
	local float CurrentHealthMax, StatDelta;
	local XComGameStateHistory History;
	...
	// Add Act scaling stats
	if (DioHQ.MissionRef.ObjectID > 0)
	{
		MissionSite = XComGameState_MissionSite(History.GetGameStateForObjectID(DioHQ.MissionRef.ObjectID));
		Act = MissionSite.MissionDifficultyParams.Act;

		for (scan = 0; scan < eStat_MAX; scan++)
		{
			StatType = ECharStatType(scan);
			StatDelta = GetMyTemplate().GetCharacterActStatGain(StatType, Act);
			if (StatDelta > 0)
			{
				SetBaseMaxStat(StatType, GetMaxStat(StatType) + StatDelta);
	...
	//Add a random amount of initiative to each unit as it spawns, to generate variance in the timeline
...
function EventListenerReturn UnitChangedTeam_Listener(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	{
		// No group switching but the playerturnorder needs to be rebuilt, 'move' the group to its current location
		`TACTICALRULES.MoveInitiativeGroup( GetGroupMembership( NewGameState ), `TACTICALRULES.FindInitiativeGroupIndex( GetGroupMembership( NewGameState ) ) );
	}

	return ELR_NoInterrupt;
}

function ClearEndOfEncounterEffects(XComGameState NewGameState)
{
	local StateObjectReference EmptyReference;

	bBreachAssigned = false;
	bDisabled = false;
	...
	ClearUnitValue( 'HasActed' );
}

// Clear any statuses/effects/states that shouldn't persist when starting the next breach
// I think we'll want to be more selective in some cases. Right now this function is
// more like a oversized hammer than a scalpel. @dakota.lemaster
function ClearEffectsBetweenBreaches(XComGameState NewGameState)
{
	ClearEndOfEncounterEffects(NewGameState);
}
...
function OnEndTacticalPlay(XComGameState NewGameState)
{
	...
	// Player units
	if (DioHQ.Squad.Find('ObjectID', ObjectID) != INDEX_NONE)
	{
		if (IsBleedingOut() || IsUnconscious())
		{
			BattleData = XComGameState_BattleData(History.GetSingleGameStateObjectForClass(class'XComGameState_BattleData'));
			...
			if (IsBleedingOut())
			{
				BattleData.LeftBehindBleedingOutUnitsLastMission.AddItem(GetReference());
			}
			if (IsUnconscious())
			{
				BattleData.LeftBehindUnconsciousUnitsLastMission.AddItem(GetReference());
			}
		}
	}
	...
	// Non-Androids recover HP after every mission
	if (!IsAndroid())
	{
		SetCurrentStat(eStat_HP, GetMaxStat(eStat_HP));
	}
...
simulated function string GetMatineeCommentPrefix()
{
	local X2SoldierClassTemplate SoldierClass;

	//Give the soldier class a chance to weigh in
	SoldierClass = GetSoldierClassTemplate();
	if (SoldierClass != none && SoldierClass.strTargetingMatineePrefix != "")
	{
		return SoldierClass.strTargetingMatineePrefix;
	}

	return GetMyTemplate().strTargetingMatineePrefix;
}
...
/// </summary>
simulated native function RequestResources(out array<string> ArchetypesToLoad);
simulated native function AddResource(string BodyPartType, name PartName, out array<string> ArchetypesToLoad);
...
private function int GetBaseSoldierAPAmount(ECombatIntelligence eComInt)
{
	...
	// AP Rebalance: all classes earn base 1 AP per rank, always [3/12/2019 dmcdonough]
	APReward = 1;
...
event OnCreation(optional X2DataTemplate InitTemplate)
{
	...
	if (IsSoldier() || m_CharTemplate.bTreatAsSoldier)
	{
	...
	AssignedVoiceID = m_CharTemplate.DefaultAppearance.nmVoice;
	class'XComCharacterVoice'.static.CheckForVoiceContent(AssignedVoiceID);
}
...
// bForceSet is currently only set when initializing unit states from the profile in TQL, 'UITacticalQuickLaunch.uc - line:290'
// in which case the content of the template could've changed since the last save, even if the template name remained the same.
// so it's necessary to re-initialize the unit state from a freshly loaded template.
function SetSoldierClassTemplate(name TemplateName, bool bForceSet = false)
{
	local UIScreenStack screenstacks;
	screenstacks = `Screenstack;
	if (TemplateName != m_SoldierClassTemplateName || bForceSet)
	{
...
function SetSoldierDataFromClassTemplate()
{
	local X2SoldierClassTemplate SoldierClassTemplate;
	SoldierClassTemplate = GetSoldierClassTemplate();

	if(SoldierClassTemplate != none && SoldierClassTemplate.bHasFullAppearance)
	{
		SoldierClassTemplate.SetCharacterAppearanceData(self);
	}
}
...
function bool HasArmorModSlot()
{
	return `DIOHQ.HasCompletedResearchByName('DioResearch_ModularArmor');
}
...
function bool HasExtraUtilitySlot()
{
	...
	ItemState = GetItemInSlot(eInvSlot_ArmorMod);
	if (ItemState != none)
	{
		return X2EquipmentTemplate(ItemState.GetMyTemplate()).bAddsUtilitySlot;
	}
...
// DIO: Additional check for valid weapon equips to account for linking and epic weapons
function bool CanSquadUnitEquipWeapon(XComGameState_Item WeaponItem)
{
	...
	if (GetSoldierClassTemplate().IsWeaponAllowedByClass(WeaponTemplate) == false)
	{
		return false;
	}

	//HELIOS BEGIN
	// Allow templates to bypass linker
	if (WeaponTemplate.bIsGenericWeapon)
	{
		return true;
	}
	// HELIOS END
	
	// Epic Weapons allowed for all units
	if (WeaponTemplate.bIsEpic)
	{
		return true;
	}

	// Normal weapons must be linked
	return WeaponItem.LinkedEntity.ObjectID == ObjectID;
}
...
function bool BuySoldierProgressionAbility(XComGameState NewGameState, int iAbilityRank, int iAbilityBranch, optional int AbilityPointCost = 0)
{
	...
	if (!SpendAbilityPoints(NewGameState, AbilityPointCost))
	{
		return bSuccess;
	}
	...
	// Event, using more common language of "promotion ability"
	`XEVENTMGR.TriggerEvent('STRATEGY_UnitBoughtPromotionAbility_Submitted', self, AbilityTemplate, NewGameState);

	return bSuccess;
}

function ResetSoldierRank(bool bResetStats = true)
{
	local int i;
	local ECharStatType StatType;
...
function protected int GetBonusWeaponAmmoFromAbilities(XComGameState_Item ItemState, XComGameState StartState)
{
	...
	//			3) completed Training abilities
	...
	for (Idx = 0; Idx < CompletedTrainingPrograms.Length; ++Idx)
	{
		TrainingTemplate = X2DioTrainingProgramTemplate(StratMgr.FindStrategyElementTemplate(CompletedTrainingPrograms[Idx]));
		if (TrainingTemplate != None && TrainingTemplate.TargetTemplateName != '')
		{
			AbilityTemplate = AbilityTemplateManager.FindAbilityTemplate(TrainingTemplate.TargetTemplateName);
			if (AbilityTemplate != none && AbilityTemplate.GetBonusWeaponAmmoFn != none)
				Bonus += AbilityTemplate.GetBonusWeaponAmmoFn(self, ItemState);
		}
	}
...
function array<AbilitySetupData> GatherUnitAbilitiesForInit(optional XComGameState StartState, optional XComGameState_Player PlayerState, optional bool bMultiplayerDisplay)
{
	...
	if (!CharacterTemplate.bSkipDefaultAbilities)
	{
		...
		//DIO - add breach abilities to each character. Future improvement: maybe limit this to certain characters based on a template flag?
		foreach class'X2Ability_BreachAbilities'.default.BreachAbilitySet(AbilityName)
		{
			AbilityTemplate = AbilityTemplateMan.FindAbilityTemplate(AbilityName);
			if (AbilityTemplate != none &&
				(!AbilityTemplate.bUniqueSource || arrData.Find('TemplateName', AbilityTemplate.DataName) == INDEX_NONE) &&
			...
			{
				`RedScreen("BreachAbilitySet array specifies unknown ability:" @ AbilityName);
			}
		}
	}
	...
	if (AbilityContext != None)
	{
		...
		if (AbilityState != None)
		{
			...
			if (AbilityTemplate != None)
			{
				...
				for (ScanEffect = 0; ScanEffect < AbilityTemplate.AbilityShooterEffects.Length; ++ScanEffect)
				{
					SpawnUnitEffect = X2Effect_SpawnUnit(AbilityTemplate.AbilityShooterEffects[ScanEffect]);
					if (SpawnUnitEffect != None)
					{
						SpawnUnitEffect.ModifyItemsPreActivation(GetReference(), StartState);
					}
		...
		foreach WeaponUpgradeTemplates(WeaponUpgradeTemplate)
		{
			foreach WeaponUpgradeTemplate.BonusAbilities(AbilityName)
			{
				   ...
				   AbilityTemplate.ConditionsEverValidForUnit(self, false) )
				{
					...
					// Handle specific case for Blueblood where both pistols get upgraded so he gets 2 of the same ability
					if (arrData.Find('TemplateName', AbilityName) == INDEX_NONE)
					{
						arrData.AddItem(Data);
					}
					else
	...
	// Gather any trained abilities
	for (i = 0; i < CompletedTrainingPrograms.Length; ++i)
	{
		TrainingTemplate = X2DioTrainingProgramTemplate(StratMgr.FindStrategyElementTemplate(CompletedTrainingPrograms[i]));
		if (TrainingTemplate == none)
		{
			continue;
		}
		if (TrainingTemplate.TargetTemplateName == '') 
		{
			continue;
		}
		AbilityTemplate = AbilityTemplateMan.FindAbilityTemplate(TrainingTemplate.TargetTemplateName);
		if (AbilityTemplate == none)
		{
			continue;
		}
		Data = EmptyData;
		Data.TemplateName = TrainingTemplate.TargetTemplateName;
		Data.Template = AbilityTemplate;
		arrData.AddItem(Data);
	}
	...
	if (PlayerState != none && PlayerState.SoldierUnlockTemplates.Length > 0)
	{
		...
		{
			AbilityUnlockTemplate = X2SoldierAbilityUnlockTemplate(StratMgr.FindStrategyElementTemplate(UnlockName));
	...
	{
		//dakota.lemaster: unnecessary Log, dio will need to provide an alternate set of strategy requirements
...
function string GetName( ENameType eType )
{
	...
	if (IsSoldier() || (IsCivilian() && !GetMyTemplate().bIsVIP) || GetMyTemplate().bTreatAsSoldier)
	{
		if( IsMPCharacter() )
		...
		case eNameType_RankFull:
			if( IsSoldier() || GetMyTemplate().bTreatAsSoldier)
			{
				if( GhostSourceUnit.ObjectID > 0 )
				{
		...
		case eNameType_FullNick:
			...
			break;
		case eNameType_RankNick:
			if (GhostSourceUnit.ObjectID > 0) return strNickName;
			return `GET_RANK_ABBRV(m_SoldierRank, m_SoldierClassTemplateName) @ strNickName;
			break;
		}
		
		return "???";
	}
	else if(eType == eNameType_First && GetMyTemplate().strShortName != "")
	{
		return GetMyTemplate().strShortName;
	}
...
{
	// DIO DEPRECATED [8/8/2019 dmcdonough]
	return "";
}
...
{
	local bool CanBleedOut, EffectsAllowBleedOut, AlreadyBleedingOut;
	local StateObjectReference EffectRef;
	local XComGameState_Effect EffectState;
	local X2Effect_Persistent EffectTemplate;
	local XComGameStateHistory History;
	...
	History = `XCOMHISTORY;
	CharacterTemplate = GetMyTemplate();
	AlreadyBleedingOut = IsBleedingOut();
	CanBleedOut = CharacterTemplate.bCanBeCriticallyWounded && `TACTICALRULES != none && !AlreadyBleedingOut && !IsUnconscious();
...
protected function bool ApplyBleedingOut(XComGameState NewGameState)
{
	...
	{
		// Apply Scar Chance
		SetScarChanceFlag(NewGameState, eScarFlag_BleedOut);

		// Create BleedOutEffect
		BleedOutEffect = class'X2StatusEffects'.static.CreateBleedingOutStatusEffect(`TACTICALDIFFICULTYSETTING);
						...
						optional bool bForceBleedOut = false, optional bool bAllowBleedout = true, optional bool bIgnoreShields = false, optional array<DamageModifierInfo> SpecialDamageMessages)
{
	...
	if (IsUnconscious()) // Unconscious units can not take further damage, they are effectively removed from combat
	{
		return;
	}
	...
	if (!bForceBleedOut && CanBleedOut())
	{
		...
		if (!bForceBleedOut && `DIO_FORCE_BLEED_OUT_IF_ABLE)
		{
			bForceBleedOut = true; //dakota: In DIO, all unit that can bleed out do so
		}
	}
	...
	{
		EventManager.TriggerEvent( 'UnitAttackedWithNoDamage', ThisObj, ThisObj, NewGameState );
	}
	...
	if (OverkillDamage <= 0)
	{
		...
		if( DamageEffect == None || !DamageEffect.bBypassSustainEffects )
		{
			if( `CHEATMGR == none || !`CHEATMGR.bSkipPreDeathCheckEffects )
			{
				...
				if (class'X2Effect_Lifeline'.static.TriggerLifelineForDamageEffect(NewGameState, self, DamageEffect, DamageSource))
				{
					`COMBATLOG("Damage Effect forces Lifeline usage - unit should be dead but the lifeline effect is handling it");
					return;
				}
			}
		}
	...
	{
		// Scar flag
		SetScarChanceFlag(NewGameState, eScarFlag_HalfHP);
...
protected function OnUnitDied(XComGameState NewGameState, Object CauseOfDeath, const out StateObjectReference SourceStateObjectRef, bool ApplyToOwnerAndComponents = true, optional const out EffectAppliedData EffectData)
{
	...
	local XComGameStateHistory History;
	local int iComponentID, AndroidIndex;
	local bool bAllDead;
	local X2EventManager EventManager;
	local string LogMsg;
	...
	// dead androids can longer be repaired. remove it from HQ
	if (IsAndroid())
	{
		DioHQ = `DioHQ;
		DioHQ = XComGameState_HeadquartersDio(NewGameState.ModifyStateObject(class'XComGameState_HeadquartersDio', DioHQ.ObjectID));
		AndroidIndex = DioHQ.Androids.Find('ObjectID', ObjectID);
		if (AndroidIndex > -1)
		{
			DioHQ.Androids.Remove(AndroidIndex, 1);
		}
	}
	...
	if( GetTeam() == eTeam_Alien || GetTeam() == eTeam_TheLost )
	{
		...
		{	
			if (Killer != none && Killer.CanEarnXp())
			{
				...
				Killer.KilledUnitsLastMission.AddItem(GetReference());
				Killer.KilledUnitsLastTurn.AddItem(GetReference());
...
{
	// DIO DEPRECATED [9/10/2019 dmcdonough]
...
//Return the state object associated with the given ability for this unit
native function XComGameState_Ability GetAbilityState(name AbilityName);
...
event int NumActionPoints(optional name Type=class'X2CharacterTemplateManager'.default.StandardActionPoint)
{
...
protected native function int InternalNumActionPoints(const out array<name> Actions, name Type);
...
function DisplayActionPointInfoFlyover()
{
	...
	if( ControllingPlayer.ObjectID == Rules.GetLocalClientPlayerObjectID() && !`REPLAY.bInReplay )
	{
		...
		{
			// display all action points remaining, if we're not in breach phase
			DioHQ = XComGameState_HeadquartersDio(History.GetSingleGameStateObjectForClass(class'XComGameState_HeadquartersDio', true));
			BattleData = XComGameState_BattleData(History.GetSingleGameStateObjectForClass(class'XComGameState_BattleData'));
			if (BattleData.bInBreachPhase == false)
			{
				if (ActionPoints.Length != 2 || DioHQ.TacticalGameplayTags.Find('DioTutorialMissionActive') != INDEX_NONE)
				{
					DisplayString = PresentationLayer.m_strActionPointsRemaining @ ActionPoints.Length;
				}
			}
		}
...
function AssignRandVoiceFromVoiceSet()
{
	local int index;
	local X2CharacterTemplate Template;
	...
	if (Template.DefaultAppearance.nmVoiceSet.length > 0)
	{
		index = `SYNC_RAND(Template.DefaultAppearance.nmVoiceSet.length);
		AssignedVoiceID = Template.DefaultAppearance.nmVoiceSet[index];
	}
}
...
{
	local int i, k;
	local bool bOwnedAbilityAtRank;
	...
	// Check if there is any rank this soldier has passed with no owned abilities
	for (i = 0; i <= m_SoldierRank; ++i)
	{
		bOwnedAbilityAtRank = false;

		RankAbilities = AbilityTree[i].Abilities;		
		for (k = 0; k < RankAbilities.Length; ++k)
		{
			if (HasSoldierAbility(RankAbilities[k].AbilityName))
			{
				bOwnedAbilityAtRank = true;
				break;
			}
		}

		// If any rank has no owned abilities yet, one can be picked
		if (!bOwnedAbilityAtRank)
		{
			return true;
		}
	}
...
// bsg-dforrest (4.10.17): support for async loading pawns with preloading all related archetypes
// Build a list of the weapon archetypes that need to be preloaded for the default (character pool) loadout
function GetDefaultLoadoutArchetypes(out array<string> ItemArchetypes)
{
	...
	// bsg-dforrest (7.19.17): Support to async preload archetypes for MP squad editor pawns
	if(`ISCONSOLE && IsMPCharacter())
	{
		if(m_MPCharacterTemplate != none && m_MPCharacterTemplate.Loadout != '')
		{
			UseLoadoutName = m_MPCharacterTemplate.Loadout;
		}
	}
	// bsg-dforrest (7.19.17): end
	else
	{
		SoldierClassTemplate = GetSoldierClassTemplate();
	...
	// find the template archetype names
	ItemTemplateManager = class'X2ItemTemplateManager'.static.GetItemTemplateManager();
	...
	if (bFoundLoadout)
	{
		foreach Loadout.Items(LoadoutItem)
		{
			...
			if (EquipmentTemplate != none)
			{
				// bsg-dforrest (7.19.17): consolidated function
				EquipmentTemplate.AddPreloadArchtypes(ItemArchetypes);
				// bsg-dforrest (7.19.17): end
			}
		}
	}
	...
	// bsg-dforrest (7.19.17): also load the required loadout if there is one
	RequiredLoadout = GetMyTemplate().RequiredLoadout;
	if (RequiredLoadout != '' && RequiredLoadout != UseLoadoutName)
	{
		foreach ItemTemplateManager.Loadouts(Loadout)
		{
			if (Loadout.LoadoutName == RequiredLoadout)
			{
				bFoundLoadout = true;
				break;
			}
	...
	// bsg-dforrest (7.19.17): go ahead and preload kevlar any time we have a soldier unit
	if(IsSoldier())
	{
...
// bsg-dforrest (4.10.17): added load voice and load item optional params
simulated function LoadPawnPackagesAsync( Actor PawnOwner, vector UseLocation, rotator UseRotation, delegate<OnUnitPawnCreated> Callback, optional bool bLoadVoice = false, optional bool bLoadItems = true )
// bsg-dforrest (4.10.17): end
{
    local XComGameState_Unit_AsyncLoadRequest alr;
    local string strArchetype;
	local int i;
	local bool bPrevLoadVoice; // bsg-dforrest (4.10.17): transient flag for preloading voice datat with RequestResources
	...
	// bsg-dforrest (4.10.17): transient flag for preloading voice datat with RequestResources
	bPrevLoadVoice = bLoadVoiceWhenRequestResources;
	bLoadVoiceWhenRequestResources=bLoadVoice;
	// bsg-dforrest (4.10.17): end

	RequestResources(alr.ArchetypesToLoad);

	// bsg-dforrest (4.10.17): get loadout archetypes when needed
	GetDefaultLoadoutArchetypes(alr.ArchetypesToLoad);
	// bsg-dforrest (4.10.17): end
	...
	`log(GetFuncName()$" "$self$" add load=["$strArchetype$"]",, 'DevAsyncPawnLoading');
	...
	for (i = 0; i < alr.ArchetypesToLoad.Length; ++i)
	{
		if (alr.ArchetypesToLoad[i] != "")
		{
			...
			`log(GetFuncName()$" "$self$" add load=["$alr.ArchetypesToLoad[i]$"]",, 'DevAsyncPawnLoading');
		}
	}
	...
	// bsg-dforrest (4.10.17): transient flag for preloading voice datat with RequestResources
	bLoadVoiceWhenRequestResources=bPrevLoadVoice;
	// bsg-dforrest (4.10.17): end
}

// bsg-dforrest (4.11.17): support to async load appearance updates
simulated function LoadArchetypesByListAsync( Object ObjectCallback, array<string> ArchetypeNames, delegate<OnUnitPawnCreated> Callback)
{
	local XComGameState_Unit_AsyncLoadRequest alr;
	local int i;
	...
	if( ArchetypeNames.Length > 0 )
	{
		...
		m_asynchronousLoadRequests.AddItem(alr);
		for( i = 0; i < ArchetypeNames.Length; i++ )
		{
			`CONTENT.RequestGameArchetype(ArchetypeNames[i], ObjectCallback, alr.OnObjectLoaded, true);
			alr.NumContentRequests++;
			`log(GetFuncName()$" "$self$" RequestGameArchetype=["$ArchetypeNames[i]$"] NumContentRequests="$alr.NumContentRequests,, 'DevAsyncPawnLoading');
		}
	}
}
...
// bsg-dforrest(2.2.17): optionally don't load pawn voice data, disabled for dropship matinee
simulated function XComUnitPawn CreatePawn( Actor PawnOwner, vector UseLocation, rotator UseRotation, optional bool bForceMenuState = false, optional bool bLoadVoice = true )
{
	...
	{	
		...
		HumanPawn.bLoadVoice = bLoadVoice; // bsg-dforrest(2.2.17): optionally don't load pawn voice data, disabled for dropship matinee
	}
...
static function GetAllAndroidUpgradeSlots(out array<EInventorySlot> Slots)
{
	Slots.AddItem(eInvSlot_AndroidCPU);
	Slots.AddItem(eInvSlot_AndroidMotor);
	Slots.AddItem(eInvSlot_AndroidChassis);
	Slots.AddItem(eInvSlot_AndroidVision);
}
...
function bool AddItemToInventory(XComGameState_Item Item, EInventorySlot Slot, XComGameState NewGameState, optional bool bAddToFront)
{
	...
	if (CanAddItemToInventory(ItemTemplate, Slot, NewGameState, Item.Quantity))
	{
		...
		if (!IsMPCharacter() && EquipmentTemplate != none && EquipmentTemplate.bAddsUtilitySlot)
		{
			NumUtilityItemSlots = GetBaseStat(eStat_UtilityItems) + 1.0f;
			SetBaseMaxStat(eStat_UtilityItems, NumUtilityItemSlots);
			SetCurrentStat(eStat_UtilityItems, NumUtilityItemSlots);
		}
...
simulated function bool CanAddItemToInventory(const X2ItemTemplate ItemTemplate, const EInventorySlot Slot, optional XComGameState CheckGameState, optional int Quantity=1)
{
	...
	if (ItemTemplate != none)
	{
		...
		case eInvSlot_ArmorMod:
			if (!HasArmorModSlot())
				return false;
			if (GetItemInSlot(eInvSlot_ArmorMod, CheckGameState) != none)
				return false;
			return (EquipmentTemplate != none && EquipmentTemplate.InventorySlot == eInvSlot_ArmorMod);
		case eInvSlot_Breach:
			if (GetItemInSlot(eInvSlot_Breach, CheckGameState) != none)
				return false;
			return (EquipmentTemplate != none && EquipmentTemplate.InventorySlot == eInvSlot_Breach);
		default:
...
public function bool RespectsUniqueRule(const X2ItemTemplate ItemTemplate, const EInventorySlot Slot, optional XComGameState CheckGameState, optional int SkipObjectID)
{
	...
	if (!class'X2TacticalGameRulesetDataStructures'.static.InventorySlotBypassesUniqueRule(Slot))
	{
		...
		// Disabling secondary weapon cat check, item cat should be sufficient [dmcdonough 12-14-19]
		//WeaponTemplate = X2WeaponTemplate(ItemTemplate);
...
simulated function bool RemoveItemFromInventory(XComGameState_Item Item, optional XComGameState ModifyGameState)
{
	...
	local X2EquipmentTemplate EquipmentTemplate;
	local X2ArmorTemplate ArmorTemplate;
	local int RemoveIndex, NumUtilityItemSlots;
	...
	{				
		...
		ItemTemplate = Item.GetMyTemplate();
		EquipmentTemplate = X2EquipmentTemplate(ItemTemplate);
		if (EquipmentTemplate != none && EquipmentTemplate.bAddsUtilitySlot && !HasExtraUtilitySlotFromAbility())      //  don't lower the bonus if one is given via an ability
		{
			NumUtilityItemSlots = GetBaseStat(eStat_UtilityItems) - 1;
			SetBaseMaxStat(eStat_UtilityItems, NumUtilityItemSlots);
			SetCurrentStat(eStat_UtilityItems, NumUtilityItemSlots);
		}
...
native function bool CanRemoveItemFromInventory(XComGameState_Item Item, optional XComGameState CheckGameState);
...
simulated function bool IsImmuneToStasis()
{
	return GetMyTemplate().bImmuneToStasis;
}

simulated native function bool IsAdvent() const;

simulated native function bool IsAlien() const;
simulated native function bool IsNonHumanoid() const;
...
simulated native function bool IsTreatedAsSoldier() const;
...
simulated native function bool IsCaptured() const;
simulated native function bool IsDowned() const;
simulated native function bool IsBerserked() const;
...
simulated native function bool IsIncapacitatedCapturedOrDowned() const;
simulated native function bool IsLifted() const;
simulated native function bool IsBound() const;
native function bool IsFleeing() const;
simulated native function bool IsImmuneToMindControl() const;
native function bool HasAssignedRoomBeenBreached() const;
native function bool IsAssignedRoomCurrentBreachRoom() const;
...
{
	return IsUnitAffectedByEffectName('HunkerDown') || IsUnitAffectedByEffectName('DelayTurnDefenseEffect');
}
...
// Needed for new TQL
simulated function ClearControllingPlayer()
{
	local StateObjectReference EmptyRef;

	ControllingPlayer = EmptyRef;
}
...
function BuildVisualizationForRankUp(XComGameState VisualizeGameState)
{
	...
	if (`DIO_DISABLE_PROMOTION_NOTIFY)
	{
		return;
	}
...
static function CreateConcealmentBreakerTracks(XComGameState VisualizeGameState, XComGameState_Unit UnitState)
{
	...
	if (`DIO_HASTEN_SPOTTED_NOTIFY)
	{
		EffectAction.CenterCameraOnEffectDuration = 0.0f;
...
function EventListenerReturn OnAbilityActivated(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	local int SuperConcealedModifier, RandRoll, Priority, LostSpawnModifier;
	local X2AbilityTemplate AbilityTemplate;
	local X2AIBTBehaviorTree BTMgr;
...
static function UnitAGainsKnowledgeOfUnitB(XComGameState_Unit UnitA, XComGameState_Unit UnitB, XComGameState AlertInstigatingGameState, EAlertCause AlertCause, bool bUnitAIsMidMove)
{
	...
	if (`DIO_SUPPRESS_KNOWLEDGE_OF_UNITS)
	{
		return;
	}
	...
	if (UnitB == none)
		return;
	if( !UnitA.HasAssignedRoomBeenBreached() || !UnitB.HasAssignedRoomBeenBreached() )
	{
		return;
	}
...
function bool UpdateAlertData(EAlertCause AlertCause, AlertAbilityInfo AlertInfo)
{
	...
	AIUnitDataID = GetAIUnitDataID();
	if( AIUnitDataID > 0 && !`DIO_DEBUG_DISABLE_ALERT_DATA_UPDATES)
	{
...
function DropCarriedLoot(XComGameState ModifyGameState)
{
	...
	local array<XComGameState_Item> Items;
	local XComGameState_Item Item;
	local array<EInventorySlot> AndroidPartSlots;
	local EInventorySlot AndroidPartSlot;
	...
	// androids have a chance to drop parts
	GetAllAndroidUpgradeSlots(AndroidPartSlots);
	foreach AndroidPartSlots(AndroidPartSlot)
	{
		Item = NewUnit.GetItemInSlot(AndroidPartSlot, ModifyGameState);

		if (Item != none)
		{
			if (`SYNC_RAND_STATIC(100) <= Item.GetMyTemplate().ChanceToDropAsLoot)
			{
				Items.AddItem(Item);
			}
			else 
	...
	foreach Items(Item)
	{
		NewUnit.RemoveItemFromInventory(Item, ModifyGameState);
	}
...
function Lootable MakeAvailableLoot(XComGameState ModifyGameState)
{
	...
	foreach NewUnit.PendingLoot.AvailableLoot(Ref)
	{
		...
		LootItems.AddItem(NewItem);
	}
	NewUnit.PendingLoot.AvailableLoot.length = 0;
	...
	class'XComGameState_LootDrop'.static.CreateLootDrop(ModifyGameState, LootItems, self, true);
...
function ApplyInventoryLoadout(XComGameState ModifyGameState, optional name NonDefaultLoadout)
{
	...
	local name UseLoadoutName, RequiredLoadout;
	local X2SoldierClassTemplate SoldierClassTemplate;
	local array<name> PrimaryWeaponTemplateNames;
	local name RandomlySelectedPrimaryWeapon;
	local bool bFoundLoadout, bTutorial;

	DioHQ = XComGameState_HeadquartersDio(`XCOMHISTORY.GetSingleGameStateObjectForClass(class'XComGameState_HeadquartersDio', true));
	if (DioHQ != none)
	{
	...
	if (bFoundLoadout)
	{
		// if there are multiple primary weapons listed in the loadout, randomly pick one
		foreach Loadout.Items(LoadoutItem)
		{
			// tutorial-only loadout items
			if (!bTutorial && LoadoutItem.bTutorialOnly)
			{
				continue;
			}
			...
			{
				PrimaryWeaponTemplateNames.AddItem(EquipmentTemplate.DataName);
			}
		}
		if (PrimaryWeaponTemplateNames.length > 0)
		{
			RandomlySelectedPrimaryWeapon = PrimaryWeaponTemplateNames[`SYNC_RAND_STATIC(PrimaryWeaponTemplateNames.length)];
		}
		...
		foreach Loadout.Items(LoadoutItem)
		{
			...
			if (EquipmentTemplate != none)
			{
				// only equip the randomly selected primary weapon
				if (EquipmentTemplate.InventorySlot == eInvSlot_PrimaryWeapon 
					&& EquipmentTemplate.DataName != RandomlySelectedPrimaryWeapon)
				{
					continue;
				}
				...
				{
					// Link weapons
					NewItem.LinkedEntity = GetReference();
...
function ValidateLoadout(XComGameState NewGameState)
{
	...
	local XComGameState_HeadquartersXCom XComHQ;
	local XComGameState_Item EquippedArmor, EquippedPrimaryWeapon, EquippedSecondaryWeapon; // Default slots
	local XComGameState_Item EquippedHeavyWeapon, EquippedGrenade, EquippedAmmo, EquippedArmorMod, UtilityItem; // Special slots
	local array<XComGameState_Item> EquippedUtilityItems; // Utility Slots
	local int idx;
	local int ExtraUtilityItemSlots;
	...
	if(EquippedSecondaryWeapon == none && NeedsSecondaryWeapon())
	{
		...
		if (EquippedSecondaryWeapon != none)
		{
			AddItemToInventory(EquippedSecondaryWeapon, eInvSlot_SecondaryWeapon, NewGameState);
		}
		else
		{
			`redscreenonce( self @ "(template" @ m_TemplateName @ ")" @ "using soldier class" @ GetSoldierClassTemplate().DataName @ "tried to equip a secondary weapon, but there was none available in its inventory. Ensure there is a valid secondary weapon in inventory, or set bNoSecondaryWeapon in" @ GetSoldierClassTemplate().DataName @ "to false." );
		}
	}
	...
	EquippedArmorMod = GetItemInSlot(eInvSlot_ArmorMod);
	if(!IsMPCharacter())
	{
		ExtraUtilityItemSlots = 0;
		
		if (EquippedArmorMod != none && X2EquipmentTemplate(EquippedArmorMod.GetMyTemplate()).bAddsUtilitySlot)
		{
			ExtraUtilityItemSlots++;
		}
		if (HasExtraUtilitySlotFromAbility())
		{
			ExtraUtilityItemSlots++;
		}
		if (ExtraUtilityItemSlots > 0)
		{
			SetBaseMaxStat(eStat_UtilityItems, GetMyTemplate().GetCharacterBaseStat(eStat_UtilityItems) + ExtraUtilityItemSlots);
			SetCurrentStat(eStat_UtilityItems, GetMyTemplate().GetCharacterBaseStat(eStat_UtilityItems) + ExtraUtilityItemSlots);
		}
		else
		{
...
function MakeItemsAvailable(XComGameState NewGameState, optional bool bStoreOldItems = true, optional array<EInventorySlot> SlotsToClear)
{
	...
	// Grab HQ Object
	foreach NewGameState.IterateByClassType(class'XComGameState_HeadquartersDio', DioHQ)
	{
		break;
	}
	...
	for(idx = 0; idx <AllItems.Length; idx++)
	{
		...
		if(bClearAll || SlotsToClear.Find(eSlot) != INDEX_NONE)
		{
			...
			{				
				DioHQ.PutItemInInventory(NewGameState, ItemState);
			}
			else
			{
...
function int GetNumVisibleEnemyUnits( bool bAliveOnly=true, bool bBreakOnAnyHits=false, bool bIncludeUnspotted=false, int HistoryIndex=-1, bool bIncludeIncapacitated=false, bool bIncludeCosmetic=false, bool bIncludeLost=true )
{
	...
	foreach VisibleUnits(kObjRef)
	{
		...
		{
			if(   ( !bIncludeIncapacitated && kEnemy.IsIncapacitatedCapturedOrDowned() )
...
//---------------------------------------------------------------------------------------
function AwardAbilityPoints(XComGameState ModifyGameState, int NumPoints)
{
	local XComGameState_Unit Unit;

	// Early out: non-pos points
	if (NumPoints <= 0)
	{
		return;
	}

	Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
	Unit.AbilityPoints += NumPoints;

	`XEVENTMGR.TriggerEvent('AbilityPointsChange', Unit, , ModifyGameState);
}

//---------------------------------------------------------------------------------------
function bool SpendAbilityPoints(XComGameState ModifyGameState, int NumPoints)
{
	local XComGameState_Unit Unit;
	...
	if (Unit.AbilityPoints >= NumPoints)
	{
		Unit.AbilityPoints -= NumPoints;
		Unit.SpentAbilityPoints += NumPoints; // Save the amount of AP spent
	}
	else
	{
...
function int AddXp(int Delta)
{
	...
	`Log("**** AddXP [" @ Delta @ "]", , 'XCom_XP');
	`Log("****** CurrentXP [" @ m_iXp @ "] (m_iXp)", , 'XCom_XP');
	`Log("****** CurrentRank [" @ m_SoldierRank @ "]", , 'XCom_XP');
	NewXp = m_iXp + Delta;
	`Log("****** NewXP [" @ NewXp @ "]", , 'XCom_XP');
	...
	RankXp = max(RankXp, 0);
	`Log("****** RankXP [" @ RankXp @ "] values for clamping to ensure we don't double-level", , 'XCom_XP');
	if (NewXp > RankXp)
	{
		ExcessXp = NewXp - RankXp;
		NewXp = RankXp;
		`Log("****** NewXP [" @ NewXp @ "] was clamped by RankXP, excess[" @ ExcessXp @ "]", , 'XCom_XP');
	}
	m_iXp = NewXp;
	m_iXp = max(m_iXp, 0);
	`Log("****** SETTING m_iXp [" @ m_iXp @ "] to NewXP", , 'XCom_XP');
	return ExcessXp;
}
...
{
	// Early outs
	if (GetSoldierClassTemplate().bBlockRankingUp)
	{
		return false;
	}
	if (bRankedUp)
	...
	if (m_SoldierRank + 1 > `GET_MAX_RANK)
	{
		return false;
	}
...
function RankUpSoldier(XComGameState NewGameState, optional name SoldierClass, optional bool bRecoveredFromBadClassData)
{
	...
	local int RankIndex, i, MaxStat, NewMaxStat, StatVal, StatCap;
	...
	if (RankIndex >= 0 && RankIndex < Template.GetMaxConfiguredRank())
	{
		...
		// Stat Progression
		StatProgression = Template.GetStatProgression(m_SoldierRank);

		// add global stat progression
		if (m_SoldierRank > 0)
		{
		...
		// update stats based on progression
		for (i = 0; i < StatProgression.Length; ++i)
		{
			...
			// set the new max stat value
			NewMaxStat = MaxStat + StatVal;
			SetBaseMaxStat(StatProgression[i].StatType, NewMaxStat);
		}

		// Training Programs
		UnlockTrainingAtRank(m_SoldierRank);

		`XEVENTMGR.TriggerEvent('UnitRankUp', self, , NewGameState);
		class'DioStrategyNotificationsHelper'.static.RefreshPromotionNotifications(NewGameState);
	}
}

function bool HasTrainingAvailableAtRank(int RankIndex)
{
	local X2SoldierClassTemplate Template;
	local array<name> TrainingPrograms;
	local int i;

	Template = GetSoldierClassTemplate();
	TrainingPrograms = Template.GetTrainingPrograms(RankIndex);

	for( i = 0; i < TrainingPrograms.Length; ++i )
	{
		if( AvailableTrainingPrograms.Find(TrainingPrograms[i]) == INDEX_NONE )
		{
			return true;
		}
	}
...
function UnlockTrainingAtRank(int RankIndex)
{
	local X2SoldierClassTemplate Template;
	local array<name> TrainingPrograms;
	local int i;
	...
	for (i = 0; i < TrainingPrograms.Length; ++i)
	{
		if (AvailableTrainingPrograms.Find(TrainingPrograms[i]) == INDEX_NONE)
		{
			AvailableTrainingPrograms.AddItem(TrainingPrograms[i]);
		}
	}
}
...
{
	return (IsAlive() && !bCaptured && HasAvailablePerksToAssign());
}
...
function EvacuateUnit(XComGameState NewGameState)
{
	...
	NewUnitState.RemoveUnitFromPlay();
	NewUnitState.bCanTraverseRooms = true;
...
function RemoveUnitFromPlay()
{
	...
	if( UnitVisualizer != none )
	{
		...
		BTMgr = `BEHAVIORTREEMGR;
		BTMgr.RemoveFromBTQueue( ObjectID );
		if( BTMgr.ActiveObjectID == ObjectID )
		{
			if( BTMgr.ActiveBTQueueEntry.ObjectID == ObjectID )
			{
				BTMgr.ActiveBTQueueEntry.RunCount = 0;
			}
			if( UnitVisualizer.m_kBehavior != None )
			{
				UnitVisualizer.m_kBehavior.OnBehaviorTreeRunComplete( true );
			}
		}
...
simulated function EUISummary_UnitStats GetUISummary_UnitStats()
{
	...
	Summary.Dodge = Min(100, int(GetCurrentStat(eStat_Dodge)));
...
simulated function array<X2Effect_Persistent> GetUISummary_UnitStatusEffects()
{
	local array<X2Effect_Persistent> List;
	local XComGameState_Effect EffectState;
	local X2Effect_Persistent Persist;
	local StateObjectReference EffectRef;
	local XComGameStateHistory History;
	...
	foreach AffectedByEffects(EffectRef)
	{
		...
		{
			List.AddItem(Persist);
		}
	}
	return List;
}

simulated function array<XComGameState_Effect> GetUISummary_GameStateEffects()
{
	local array<XComGameState_Effect> List;
	local XComGameState_Effect EffectState;
	local X2Effect_Persistent Persist;
	local StateObjectReference EffectRef;
	local XComGameStateHistory History;
	...
	foreach AffectedByEffects(EffectRef)
	{
		...
		if (Persist.bDisplayInUI && Persist.IsEffectCurrentlyRelevant(EffectState, self))
		{
			List.AddItem(EffectState);
		}
	}
	return List;
}

simulated function array<string> GetUISummary_UnitStatusIcons(optional bool bOnlyUITimelineIcons = false)
{
	...
	foreach AffectedByEffects(EffectRef)
	{
		EffectState = XComGameState_Effect(History.GetGameStateForObjectID(EffectRef.ObjectID));
		Persist = EffectState.GetX2Effect();
		if( Persist.bDisplayInUI && Persist.IsEffectCurrentlyRelevant(EffectState, self) && Persist.StatusIcon != "")
		{
			if( bOnlyUITimelineIcons == false || Persist.bDisplayStatusIconInUITimeline == true )
			{
...
simulated function GetUISummary_TargetableUnits(out array<StateObjectReference> arrVisibleUnits, out array<StateObjectReference> arrSSEnemies, out array<StateObjectReference> arrCurrentlyAffectable, XComGameState_Ability CurrentAbility, int HistoryIndex)
{
	...
	// mmg_mike.anstine TODO - Does hacking still work?
	// bsg-jrebar 7/29/16 Removing hackables from available targets list unless in the hack menu
	if (!`ISCONSOLE) //bsg-jhilton (08.15.2016) - Wrap this instead of commenting it out to fix up script warnings
	{		
...
simulated function float GetMaxArmor()
{
	local float Total;
	local XComGameStateHistory History;
	local XComGameState_Effect EffectState;
...
event bool DoesBlockUnitPathing(const XComGameState_Unit TestUnit)
{
	...
	if( ( IsCivilian() || ( GetTeam() == eTeam_XCom ) ) && TestUnit.GetTeam() == eTeam_XCom )
	{
		return false;
	}
	...
	// units that are traversing rooms, such as reinforcement or fleeing units, can't be blocked
	if (TestUnit.bCanTraverseRooms)
	{
		return false;
	}
...
native function bool ShouldAvoidHazardsWhenPathing();
...
//---------------------------------------------------------------------------------------
// Check if this unit meets the requirements to be counted as 'captured' when a mission ends
function bool IsCapturedOnEndBattle()
{
	if (GetTeam() == eTeam_XCom)
	{
		// Mind-controlled enemies are capturable
		if (!IsMindControlled())
		{
			return false;
		}
	...
	if (IsCivilian())
	{
		if (!GetMyTemplate().bIsVIP)
		{
			return false;
		}
	}

	return IsUnconscious() || IsBound();
}
...
// Perform all upkeep functions that occur at the start of each strategy turn
simulated function DoTurnUpkeep(XComGameState ModifyGameState)
{
	...
	if (Unit.CanBecomeUnassigned(ModifyGameState))
	{
		Unit.SetAssignment(eAssignment_Unassigned);

		// If APC roster is light, add this unit automatically
		if (class'DioStrategyAI'.static.CanAssignToAPC(Unit.GetReference()))
		{
			`DIOHQ.SetUnitOnAPC(ModifyGameState, Unit.GetReference());
		}
	}
	else
	{
		// If unit can't be unassigned, mark it spent so nothing else tries to use it
		Unit.SetAssignment(eAssignment_Spent);
	}
}

//---------------------------------------------------------------------------------------
simulated function EStrategyActionAssignment GetAssignment()
{
	return HQAssignment;
}

//---------------------------------------------------------------------------------------
simulated function SetAssignment(EStrategyActionAssignment Assignment)
{
	HQAssignment = Assignment;		
}

//---------------------------------------------------------------------------------------
simulated function XComGameState_StrategyAction GetAssignedAction(optional XComGameState ModifyGameState = none)
{
	local XComGameStateHistory History;
	local XComGameState_StrategyAction Action;

	// Early out: unit is unassigned
	if (GetAssignment() == eAssignment_Unassigned)
	{
		return none;
	}

	// In case the action has been modified this frame, check ModifyGameState first
	if (ModifyGameState != none)
	{
		foreach ModifyGameState.IterateByClassType(class'XComGameState_StrategyAction', Action)
		{
			if (IsAssignedToAction(Action))
			{
				return class'DioStrategyAI'.static.GetStrategyAction(Action.GetReference(), ModifyGameState);
			}
		}
	}
...
//---------------------------------------------------------------------------------------
simulated function bool IsAssignedToAction(XComGameState_StrategyAction Action)
{
	if (Action.bRemoved)
	{
		return false;
	...
	return Action.AssignedUnitRefs.Find('ObjectID', ObjectID) != INDEX_NONE;
}

//---------------------------------------------------------------------------------------
// Returns if the unit can be assigned to a squad action right now
simulated function bool CanBeAssigned(optional StateObjectReference ToActionRef, optional XComGameState ModifyGameState=none)
{
	local XComGameState_Unit Unit;
	local XComGameStateHistory History;
	local XComGameState_StrategyAction Action;

	Unit = GetUnit(ModifyGameState);

	// Unit already acted this round
	if (Unit.HQAssignment == eAssignment_Spent || Unit.HQAssignment == eAssignment_Blocked)
	{
		return false;
	}

	// Unit already committed to an action in progress
	Action = Unit.GetAssignedAction(ModifyGameState);
	if (Action != none)
	{
	...
	// Check can assign to input action
	History = `XCOMHISTORY;
	Action = XComGameState_StrategyAction(History.GetGameStateForObjectID(ToActionRef.ObjectID));
	if (Action != none)
	{
		if (!Action.CanAssignUnit(GetReference()))
		{
			return false;
		}
	}
...
//---------------------------------------------------------------------------------------
// Check the unit's injuries/statuses to determine if it can be made available for assignment on a new strategy round
simulated function bool CanBecomeUnassigned(optional XComGameState ModifyGameState=none)
{
	local XComGameState_Unit Unit;
	local XComGameState_StrategyAction Action;
	...
	// Unit committed to an action in progress, can't leave until it's done
	Action = Unit.GetAssignedAction(ModifyGameState);
	if (Action != none)
	{
		if (Action.IsInProgress())
...
//---------------------------------------------------------------------------------------
simulated function bool IsAndroid()
{
	return GetSoldierClassTemplateName() == 'SoldierClass_Android';
}
...
// Entry point when this unit should gain scar damage: either deepen an existing scar or build and add a new one
simulated function ProgressScars(XComGameState ModifyGameState, EScarSourceFlags Source, int MissionObjectID)
{
	...
	// End HELIOS Issue #59
	{
		...
		{
			Scar = XComGameState_UnitScar(`XCOMHISTORY.GetGameStateForObjectID(Scars[i].ObjectID));
			if (Scar.CanDeepenScar())
			{
				DeepenScarOptions.AddItem(Scar.GetReference());
			}
		}

		// Deepen random scar
		if (DeepenScarOptions.Length > 0)
		{
			RandIdx = `SYNC_RAND(DeepenScarOptions.Length);
			Scar = XComGameState_UnitScar(ModifyGameState.ModifyStateObject(class'XComGameState_UnitScar', DeepenScarOptions[RandIdx].ObjectID));
			Scar.DeepenScar(ModifyGameState);
		}
	}
	...
	if (Scar != none)
	{
		Scar.AddHistory(ModifyGameState, Source, MissionObjectID);
		class'DioStrategyNotificationsHelper'.static.RefreshScarNotifications(ModifyGameState);
	}
}

//---------------------------------------------------------------------------------------
// Select (or specify) a new scar to create and add
simulated function XComGameState_UnitScar BuildNewScar(XComGameState ModifyGameState, optional X2DioUnitScarTemplate ScarTemplate)
{
	local XComGameState_UnitScar Scar;
	local XComGameState_Unit Unit;

	// Use argument template or ask for a new one
	if (ScarTemplate == none)
	{
		ScarTemplate = class'DioStrategyAI'.static.SelectNextUnitScar(GetReference());
	}

	if (ScarTemplate != none)
	{
		// Early out: make sure we don't already have a scar of this type
		if (HasScarOfTemplate(ScarTemplate))
		{
			return GetScarByTemplate(ScarTemplate, ModifyGameState);
		}

		Scar = ScarTemplate.CreateInstanceFromTemplate(ModifyGameState);
		`assert(Scar != none);

		// Apply scar to unit
		Scar.ApplyScar(ModifyGameState, GetReference());
		
		// Unit reference scar
		Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
		Unit.Scars.AddItem(Scar.GetReference());
		Unit.LastRoundScarAdded = `THIS_TURN;

		// Add scar training template to available training
		if (ScarTemplate.ScarTrainingTemplateName != '' && 
			Unit.AvailableTrainingPrograms.Find(ScarTemplate.ScarTrainingTemplateName) == INDEX_NONE)
		{
			Unit.AvailableTrainingPrograms.AddItem(ScarTemplate.ScarTrainingTemplateName);
		}
		
		`XEVENTMGR.TriggerEvent('STRATEGY_NewUnitScarEarned_Submitted', Scar, Unit, ModifyGameState);
		return Scar;
	}
...
//---------------------------------------------------------------------------------------
simulated function RemoveScar(XComGameState ModifyGameState, StateObjectReference ScarRef)
{
	local XComGameState_Unit Unit;
	local XComGameState_UnitScar Scar;
	local int ScarIndex;

	ScarIndex = Scars.Find('ObjectID', ScarRef.ObjectID);
	if (ScarIndex == INDEX_NONE)
	{
		return;
	}

	Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
	Unit.Scars.Remove(ScarIndex, 1);

	Scar = XComGameState_UnitScar(ModifyGameState.ModifyStateObject(class'XComGameState_UnitScar', ScarRef.ObjectID));
	Scar.UnapplyScar(ModifyGameState);

	`XEVENTMGR.TriggerEvent('UnitScarRemoved', Scar, Unit, ModifyGameState);

	ModifyGameState.RemoveStateObject(ScarRef.ObjectID);
}

//---------------------------------------------------------------------------------------
simulated function XComGameState_UnitScar GetScarByTemplate(X2DioUnitScarTemplate ScarTemplate, optional XComGameState ModifyGameState=none)
{
	local XComGameStateHistory History;
	local XComGameState_UnitScar Scar;
	local int i;
	...
	for (i = 0; i < Scars.Length; ++i)
	{
		Scar = XComGameState_UnitScar(History.GetGameStateForObjectID(Scars[i].ObjectID));
		if (Scar.GetMyTemplateName() != ScarTemplate.DataName)
		{
			continue;
		}
		...
		if (ModifyGameState != none)
		{
			return XComGameState_UnitScar(ModifyGameState.ModifyStateObject(class'XComGameState_UnitScar', Scar.ObjectID));
		}
		else
		{
			return Scar;
...
//---------------------------------------------------------------------------------------
simulated function bool HasScarOfTemplate(X2DioUnitScarTemplate ScarTemplate)
{
	local XComGameStateHistory History;
	local XComGameState_UnitScar Scar;
	local StateObjectReference Ref;
	...
	foreach Scars(Ref)
	{
		Scar = XComGameState_UnitScar(History.GetGameStateForObjectID(Ref.ObjectID));
		if (Scar.GetMyTemplateName() == ScarTemplate.DataName)
		{
			return true;
		}
...
simulated function SetScarChanceFlag(XComGameState ModifyGameState, EScarChanceFlags eFlag)
{
	local XComGameState_Unit Unit;

	if (ActiveScarFlags.Find(eFlag) == INDEX_NONE)
	{
		Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
		Unit.ActiveScarFlags.AddItem(eFlag);
	}
}

//---------------------------------------------------------------------------------------
simulated function ResetScarChanceFlags(XComGameState ModifyGameState)
{
	local XComGameState_Unit Unit;
	Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
	Unit.ActiveScarFlags.Length = 0;
}
...
simulated function AddSpecialty(XComGameState ModifyGameState, name SpecialtyTemplateName)
{
	local XComGameState_Unit Unit;
	local X2DioUnitSpecialtyTemplate SpecialtyTemplate;

	SpecialtyTemplate = X2DioUnitSpecialtyTemplate(`STRAT_TEMPLATE_MGR.FindStrategyElementTemplate(SpecialtyTemplateName));
	if (SpecialtyTemplate == none)
	{
		return;
	}

	Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
	if (Unit.Specialties.Find(SpecialtyTemplateName) == INDEX_NONE)
	{
		Unit.Specialties.AddItem(SpecialtyTemplateName);
		`XEVENTMGR.TriggerEvent('STRATEGY_UnitSpecialtyAdded_Submitted', SpecialtyTemplate, Unit, ModifyGameState);
	}
}

//---------------------------------------------------------------------------------------
simulated function RemoveSpecialty(XComGameState ModifyGameState, name SpecialtyTemplateName)
{
	local XComGameState_Unit Unit;
	local X2DioUnitSpecialtyTemplate SpecialtyTemplate;
	local int Index;
	...
	Unit = XComGameState_Unit(ModifyGameState.ModifyStateObject(class'XComGameState_Unit', ObjectID));
	Index = Unit.Specialties.Find(SpecialtyTemplateName);
	if (Index != INDEX_NONE)
	{
		Unit.Specialties.Remove(Index, 1);
		`XEVENTMGR.TriggerEvent('STRATEGY_UnitSpecialtyRemoved_Submitted', SpecialtyTemplate, Unit, ModifyGameState);
	}
}

// Generate cover effect can't process its own removal / destruction
function EventListenerReturn OnGeneratedCoverRemoved(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	local XComGameState NewGameState;
	local XComGameState_Unit UnitState;
	...
	if (UnitState != none)
	{
		...
		NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("OnGeneratedCoverRemoved");
		class'X2Effect_GenerateCover'.static.UpdateWorldCoverData(UnitState, NewGameState);
		`TACTICALRULES.SubmitGameState(NewGameState);

		// convenient way to trigger off all the update that need to happen following world rebuild
		`XEVENTMGR.TriggerEvent('ActiveUnitChanged', UnitState);
	}
...
DefaultProperties
{
	...
	bCanTraverseRooms=false
	LastRoundScarAdded=-1
	// bsg-dforrest (4.10.17): transient flag for preloading voice datat with RequestResources
	bLoadVoiceWhenRequestResources=false
	// bsg-dforrest (4.10.17): end
}
