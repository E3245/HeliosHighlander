...
static event class<GameInfo> SetGameType(string MapName, string Options, string Portal)
{
	...
	else if (InStr(MapName, `MAPS.StrategyMap, , true) != INDEX_NONE || InStr(GameInfoClassName, "XComHeadquartersGame", true, true) != INDEX_NONE)
	{
	...
	// HELIOS BEGIN
	// From robojumper's End of the Cycle mod. Replace any previous GameInfoClass with our custom one
	// https://github.com/X2CommunityCore/X2WOTCCommunityHighlander/commit/e7e773694d5f51489b76646b604dd7b088be22eb
...
//HELIOS BEGIN
// Allow custom gameinfos to write a custom header for saves
function string GetSaveGameHeader(string SaveType);
// HELIOS END
function string GetSavedGameDescription();
function string GetSavedGameCommand();
function string GetSavedGameMapName();
function string GetSavedGameMapBriefingImage();
...
simulated function DelayedEndOnlineGameComplete(name SessionName,bool bWasSuccessful)
{
	...
	GameInterface.DestroyOnlineGame(OnlineEventMgr.LocalUserIndex, PlayerReplicationInfoClass.default.SessionName);
	//</workshop>	
}
...
event PreBeginPlay()
{
	...
	//bsg-mfawcett(09.13.16): now that we have loaded into a tactical match, disable our hack flag
	if( GameRuleset != none && GameRuleset.IsA('X2TacticalGameRuleset') )
	{
		`XENGINE.bHACK_DisableGeoscapePauseState = false;
	}
}
