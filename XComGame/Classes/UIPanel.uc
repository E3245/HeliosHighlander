...
simulated function UIPanel AnchorCenterRight() { return SetAnchor(class'UIUtilities'.const.ANCHOR_MIDDLE_RIGHT); }
...
// HELIOS BEGIN
// Invokes the Actionscript command to set the alpha of an element that's not exposed to Unrealscript
// Arg 1: Requires the path to the element. Usually MCPath $ ".theElementYouWantToChange"
// Arg 2: Integer value of the new alpha
simulated function AS_SetMCAlpha(string ClipPath, int NewAlpha)
{
	Movie.ActionScriptVoid("Panel.setAlpha");
}
// HELIOS END
